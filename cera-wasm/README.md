...
> **Note:** Version 0.6.0 introduces consolidated session lifecycle management, transactional multi-turn chat coordination (`ChatSession`), native JSON Schema compilation, first-class tool calling, and WebGPU session checkpoint persistence. See [Releases](https://github.com/hyeons-lab/cera/releases/tag/v0.6.0).
...
Version 0.6.0 provides these capabilities alongside `ChatSession` and WebGPU checkpointing. The loader accepts owned
