...
def create_phi3_model(out_path, seed=42):
    np.random.seed(seed)
    arch = "phi3"
    ...
    k_dim = n_head_kv * head_dim # 32
    v_dim = n_head_kv * head_dim # 32
    qkv_dim = q_dim + k_dim + v_dim # 128
    n_ff = 128
    n_layers = 4
    vocab_size = 260
    ...
    # Vocabulary and GPT-2 / Phi-3 tokenizer setup
    ...
    writer.add_tokenizer_pre("phi-3")
    ...
    for i in range(n_layers):
        ...
        # Fused QKV projection and bias
        writer.add_tensor(f"blk.{i}.attn_qkv.weight", rand_weight(qkv_dim, n_embd))
        writer.add_tensor(f"blk.{i}.attn_qkv.bias", rand_weight(qkv_dim))
        writer.add_tensor(f"blk.{i}.attn_output.weight", rand_weight(n_embd, q_dim))

        writer.add_tensor(f"blk.{i}.ffn_norm.weight", np.ones((n_embd,), dtype=np.float32))
        # Packed FFN up projection (gate and up stacked row-wise)
        writer.add_tensor(f"blk.{i}.ffn_up.weight", rand_weight(2 * n_ff, n_embd))
