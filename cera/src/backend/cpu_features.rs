...
impl CpuFeatures {
    ...
    /// host that has it. That happened once already, when `fp16` was added, so
    /// treat the descriptor as a grouping key within a release rather than a
    /// join key across them.
...
mod tests {
    ...
    fn descriptor_is_compact_sorted_and_never_empty() {
        ...
        // which moved the string on every real aarch64 host. Anything keying
