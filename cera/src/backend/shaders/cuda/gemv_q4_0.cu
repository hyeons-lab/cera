...
// - Bitwise register unpacking of 4-bit nibbles with FP32 register accumulation.
...
// Grid: (ceil(m / (blockDim.x / 32)), 1, 1)
// Block: (128 or 256, 1, 1) -> 4 or 8 warps per block
__global__ void gemv_q4_0(
    ...
    // Each thread in the warp processes blocks with stride 32
    for (uint32_t ib = lane; ib < nb; ib += 32) {
        const uint8_t* blk = row_ptr + (size_t)ib * 18;

        // Scale factor: 2 bytes half float
        uint16_t d_half;
