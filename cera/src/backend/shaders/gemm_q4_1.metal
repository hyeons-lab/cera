...
// GEMM for Q4_1 weights using simdgroup matrix multiply-accumulate.
//
// Identical tiling to gemm_q4_0.metal — the only difference is the block format
// and its dequantization: Q4_1 stores an explicit minimum `m` alongside the
// scale `d`, and its nibbles are NOT recentered (`w = d·q + m`, `q ∈ [0,15]`),
// where Q4_0 uses `w = d·(q − 8)`. Because this kernel dequantizes each weight
// block to `half` and feeds a dense simdgroup MMA, the `+m` offset is simply
// baked into the dequantized value — there is no separate correction term (that
// is only needed by the int8 CPU path, which keeps weights quantized).
//
...
void dequantize_q4_1(device const block_q4_1 * xb, short il, thread half4x4 & reg) {
    // qs starts 4 bytes (two uint16: d, m) into the block.
...
kernel void gemm_q4_1(
    ...
    device const block_q4_1 * x = (device const block_q4_1 *)(src0
