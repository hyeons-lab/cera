...
pub struct GpuContext {
    ...
    pub backend: String,
    /// Whether `SHADER_F16` was available and requested. Informational:
    /// wgpu hides this flag on drivers with working Float16 (Adreno 830),
    /// so fp16 passthrough kernels must NOT gate on it (see
...
impl GpuContext {
    ...
    pub async fn new_async() -> Result<Self> {
        ...
        if std::env::var("CERA_GPU_HOST_PROFILE").as_deref() == Ok("1") {
            let f = adapter.features();
            eprintln!(
                "[GPU-HOST] adapter={adapter_name} backend={backend} SUBGROUP={} SUBGROUP_BARRIER={} TIMESTAMP={} SHADER_F16={}",
        ...
        Ok(Self {
            ...
            has_subgroup: features.contains(wgpu::Features::SUBGROUP),
    ...
    /// Fast Q4_0 decode GEMV via SPIR-V passthrough: the subgroup-reduction
    /// twin (`spirv/gemv_q4_0_fast.slang`, entry `main`). Same 4-binding
    /// interface as the WGSL twin (0 = weights, 1 = x, 2 = y, 3 = params),
    /// same NR=8 dispatch — only the module source differs. Passthrough is
    /// the only way to ship subgroups: naga cannot parse WGSL `enable
    /// subgroups`, so the WGSL twin carries the 6-barrier tree instead.
    ///
    /// # Safety
    /// The module must be spirv-val-clean with the binding interface above
    /// (our slangc-compiled `gemv_q4_0_fast.slang`). Only call when
    /// `supports_spirv_passthrough()` is true.
    pub fn gemv_q4_0_fast_passthrough(&self) -> wgpu::ComputePipeline {
        assert!(
            self.supports_spirv_passthrough(),
            "SPIR-V passthrough pipeline requested on a backend that does not \
             accept it (backend={}); gate on GpuContext::supports_spirv_passthrough()",
        ...
        let bgl = self
            .device
            .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("gemv_q4_0_fast_passthrough_bgl"),
                entries: &[
                    storage(0, true),
                    storage(1, true),
                    storage(2, false),
        ...
        let layout = self
            .device
            .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("gemv_q4_0_fast_passthrough_layout"),
                bind_group_layouts: &[Some(&bgl)],
                immediate_size: 0,
            });
        // SAFETY: slangc-compiled from gemv_q4_0_fast.slang, spirv-val clean.
        let module = unsafe {
            self.device
        ...
        self.device
            .create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
                label: Some("gemv_q4_0_fast_passthrough"),
                layout: Some(&layout),
                module: &module,
                entry_point: Some("main"),
                compilation_options: wgpu::PipelineCompilationOptions {
                    // The kernel writes `sg_scratch` (one float per subgroup
                    // per row) before the combine reads it — same contract as
                    // the naga path (see `create_pipeline_with_defines`).
                    zero_initialize_workgroup_memory: false,
                    ..Default::default()
                },
                cache: None,
    ...
    /// # Safety
    /// The module must be spirv-val-clean with the binding interface above
    /// (our slangc-compiled `gemv_q6_k.slang`). Only call when
    /// `supports_spirv_passthrough()` is true.
    pub fn gemv_q6_k_passthrough(&self) -> wgpu::ComputePipeline {
        assert!(
            self.supports_spirv_passthrough(),
            "SPIR-V passthrough pipeline requested on a backend that does not \
             accept it (backend={}); gate on GpuContext::supports_spirv_passthrough()",
        ...
        let bgl = self
            .device
            .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("gemv_q6_k_passthrough_bgl"),
                entries: &[
                    storage(0, true),
                    storage(1, true),
                    storage(2, false),
        ...
        let layout = self
            .device
            .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("gemv_q6_k_passthrough_layout"),
                bind_group_layouts: &[Some(&bgl)],
                immediate_size: 0,
            });
        // SAFETY: slangc-compiled from gemv_q6_k.slang, spirv-val clean.
        let module = unsafe {
            self.device
                .create_shader_module_passthrough(wgpu::include_spirv_raw!(concat!(
                    env!("OUT_DIR"),
        ...
        self.device
            .create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
                label: Some("gemv_q6_k_passthrough"),
                layout: Some(&layout),
                module: &module,
                entry_point: Some("main"),
                compilation_options: wgpu::PipelineCompilationOptions {
    ...
    /// Streaming Q4_0 prefill GEMM via SPIR-V passthrough. Five bindings
    /// (0 = repacked nibbles, 1 = repacked scales, 2 = f32 activations,
    /// 3 = dst, 4 = params); fp16 arithmetic inside (see `use_stream_gemm`
    /// for why that needs no capability gate beyond passthrough).
    ///
    /// # Safety
    /// `desc` must be a spirv-val-clean compute module whose binding
    /// interface matches the layout above (our slangc-compiled
    /// `gemm_stream_q4_0.slang`). Only call when
    /// `supports_spirv_passthrough()` is true.
    pub fn gemm_stream_q4_0_passthrough(&self) -> wgpu::ComputePipeline {
        assert!(
            self.supports_spirv_passthrough(),
            "SPIR-V passthrough pipeline requested on a backend that does not \
             accept it (backend={}); gate on GpuContext::supports_spirv_passthrough()",
        ...
        let bgl = self
            .device
            .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("gemm_stream_q4_0_passthrough_bgl"),
                entries: &[
                    storage(0, true),
                    storage(1, true),
                    storage(2, true),
        ...
        let layout = self
            .device
            .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("gemm_stream_q4_0_passthrough_layout"),
                bind_group_layouts: &[Some(&bgl)],
                immediate_size: 0,
            });
        // SAFETY: slangc-compiled from gemm_stream_q4_0.slang, spirv-val clean.
        let module = unsafe {
            self.device
                .create_shader_module_passthrough(wgpu::include_spirv_raw!(concat!(
                    env!("OUT_DIR"),
                    "/gemm_stream_q4_0.spv"
                )))
        };
        self.device
            .create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
                label: Some("gemm_stream_q4_0_passthrough"),
                layout: Some(&layout),
                module: &module,
                entry_point: Some("main"),
                compilation_options: wgpu::PipelineCompilationOptions {
    ...
    /// `desc` must be a spirv-val-clean compute module whose binding
    /// interface matches the layout above (our slangc-compiled
    /// `transpose_cast_f16.slang`). Only call when
    /// `supports_spirv_passthrough()` is true.
    pub fn transpose_cast_f16_passthrough(&self) -> wgpu::ComputePipeline {
        assert!(
            self.supports_spirv_passthrough(),
            "SPIR-V passthrough pipeline requested on a backend that does not \
             accept it (backend={}); gate on GpuContext::supports_spirv_passthrough()",
        ...
        let bgl = self
            .device
            .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("transpose_cast_f16_passthrough_bgl"),
                entries: &[storage(0, true), storage(1, false), storage(2, true)],
            });
        let layout = self
            .device
            .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("transpose_cast_f16_passthrough_layout"),
                bind_group_layouts: &[Some(&bgl)],
                immediate_size: 0,
            });
        // SAFETY: slangc-compiled from transpose_cast_f16.slang, spirv-val clean.
        let module = unsafe {
            self.device
                .create_shader_module_passthrough(wgpu::include_spirv_raw!(concat!(
                    env!("OUT_DIR"),
                    "/transpose_cast_f16.spv"
                )))
        };
        self.device
            .create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
                label: Some("transpose_cast_f16_passthrough"),
                layout: Some(&layout),
                module: &module,
                entry_point: Some("main"),
                compilation_options: wgpu::PipelineCompilationOptions {
...
pub mod shaders {
    ...
    /// backend's `metal::shaders::ELEMENTWISE_SLANG`. Same
    /// binding contract as [`ELEMENTWISE`].
    ///
    /// **Not** the production constant, though on this side it could be: the
    /// port covers every entry point [`ELEMENTWISE`] has. The blocker is the
    /// Metal twin, whose handwritten kernel carries four more
    /// (`memcpy_f32`, `scale_f32`, `mul_out`, `cast_f32_to_f16`) that have no
    /// WGSL counterpart to share a body with, so swapping it in there would
    /// fail at pipeline creation. Flipping one backend and not the other would
    /// give up the single shared source that is the point of the migration.
    ...
    /// the Metal backend's `metal::shaders::RMSNORM`. A
    /// `__target_switch` port that diverges in both reduction and I/O model:
    /// this wgsl branch is in-place (3 bindings) with a shared-memory tree; the
    /// metal branch is out-of-place (4 buffers) with a two-stage `simd_sum`.
    ...
    /// metal branch the two-stage `simd_sum`.
    /// `tests/slang_multitarget_parity.rs` pins both entry points against the CPU
    /// reference.
    pub const RMSNORM_BATCH: &str = include_str!(concat!(env!("OUT_DIR"), "/rmsnorm_batch.wgsl"));
    ...
    /// `metal::shaders::CONV1D_FUSED_BATCH`. A clean single-body
    ...
    /// reduction. They did differ in loop spelling (the WGSL one had no C-style
    /// `for`), in the weight preload bound, and in whether they carried the
    /// `ks > 4 || d_conv > 3` early-out (the WGSL twin did, the Metal one did
    ...
    /// two-stage `simd_sum`. `tests/slang_multitarget_parity.rs` pins it against
    /// the CPU reference.
    ...
    /// hand-written, and byte-identical in contract (binding 0 = x in-place,
    ...
    /// with the Metal backend's `metal::shaders::ARGMAX_F32`. A
    /// `__target_switch` port: this wgsl branch keeps the shared-memory tree, the
    /// metal branch the two-stage `simd_shuffle_down`.
    /// `tests/slang_multitarget_parity.rs` pins it against the CPU reference.
    pub const ARGMAX_F32: &str = include_str!(concat!(env!("OUT_DIR"), "/argmax_f32.wgsl"));
    /// RoPE (NEOX + interleaved + freq_factors), generated from
    /// `shaders/slang/rope.slang` by build.rs. A `__target_switch` port: this
    /// wgsl branch carries the full kernel; the metal branch (shared with
    /// `metal::shaders::ROPE`) is the minimal NEOX-only one.
    /// `tests/slang_multitarget_parity.rs` pins it against the CPU reference.
    ...
    /// the Metal backend's `metal::shaders::CONV1D`. Unlike the
    ...
    /// `tests/slang_multitarget_parity.rs` pins it against the CPU reference.
    pub const CONV1D: &str = include_str!(concat!(env!("OUT_DIR"), "/conv1d.wgsl"));
    /// Generated from `shaders/slang/conv1d_fused.slang` by build.rs and shared
    /// with the Metal backend's `metal::shaders::CONV1D_FUSED`.
    /// A clean single-body port with no `__target_switch`, made possible by first
    /// consolidating the Metal twin onto the WGSL twin's single packed `proj`
    /// binding. The port carries no kernel-size guard: nothing here indexes a
    /// fixed-size array, so a bound could only convert a correct result into a
    /// skipped write that leaves the output stale. ([`CONV1D_FUSED_BATCH`] does
    /// constrain its params, because it stages the weights and rolling state in
    /// fixed-size registers.) `tests/slang_multitarget_parity.rs` pins it against
    /// the CPU reference.
    pub const CONV1D_FUSED: &str = include_str!(concat!(env!("OUT_DIR"), "/conv1d_fused.wgsl"));
    // Vision-encoder (ViT) kernels.
    /// Generated from `shaders/slang/layernorm_batch.slang` by build.rs and
    /// shared with the Metal backend's
    /// `metal::shaders::LAYERNORM_BATCH`. A `__target_switch`
    /// port: this wgsl branch keeps the shared-memory tree, the metal branch the
    /// two-stage `simd_sum`. `tests/slang_multitarget_parity.rs` pins it against
    /// the CPU reference.
    ...
    /// source with the Metal backend's `metal::shaders::GELU`.
    /// Same binding contract. `tests/slang_multitarget_parity.rs` pins it against
    /// the CPU reference.
    pub const GELU: &str = include_str!(concat!(env!("OUT_DIR"), "/gelu.wgsl"));
    /// Generated from `shaders/slang/bias_add.slang` by build.rs, sharing that
    /// source with the Metal backend's
    /// `metal::shaders::BIAS_ADD`. Same binding contract.
    /// `tests/slang_multitarget_parity.rs` pins it against the CPU reference.
    pub const BIAS_ADD: &str = include_str!(concat!(env!("OUT_DIR"), "/bias_add.wgsl"));
...
mod tests {
    ...
    fn test_gpu_gelu_parity() {
        ...
        // The generated gelu kernel must keep parity with the CPU's saturating f32::tanh.
        let n = 1000usize;
    ...
    fn test_gpu_gemv_q6_k() {
        ...
        // Both module sources (WGSL always; SPIR-V passthrough where the
        // backend accepts it) against the same CPU reference. k=512 (nb=2)
        ...
        for (label, passthrough) in [("wgsl", false), ("passthrough", true)] {
            ...
            for i in 0..m as usize {
                ...
                assert!(
                    rel < 5e-3,
                    "Q6_K GEMV [{label}] mismatch at row {i}: cpu={}, gpu={}, rel={rel:.2e}",
                    expected[i],
                    result[i]
                );
            }
    ...
    /// Q8_0 register-tiled GEMM parity: synthesize Q8_0 blocks, dequantize on CPU,
    /// run the reg-tiled kernel with the Q8_0 shmem loader over a multi-token batch,
    /// compare.
    ///
    ...
    fn test_gpu_gemv_quant_dispatch_smoke() {
        ...
        struct Case<'a> {
            ...
            shader: &'static str,
            entry: &'static str,
            /// Build the pipeline via SPIR-V passthrough instead of WGSL.
            passthrough: bool,
            rows_per_wg: u32,
            weight_bytes: &'a [u8],
            expected: &'a [f32],
        ...
        let cases = [
            ...
            Case {
                ...
                passthrough: ctx.supports_spirv_passthrough() && ctx.has_subgroup,
                rows_per_wg: 8,
        ...
        for case in cases {
            let pipeline = if case.passthrough {
                ...
                ctx.create_pipeline(case.shader, case.entry, case.name)
            };
    ...
    /// per-vector `RMSNORM` kernel invoked N times. Same fixture, same
    ...
    fn test_gpu_rmsnorm_batch_parity() {
        ...
        // ─── Reference: run the per-vector RMSNORM kernel N times ───
    ...
    /// Single-token fused conv parity: the `CONV1D_FUSED` kernel (decode
    ...
    /// 32-key online-softmax tiles (with the boundary landing mid-tile) matches
    ...
    fn test_gpu_attention_prefill_long_kv() {
        ...
        // start_pos 900 + 200 queries → last query seq_len 1100 > 34 × TILE(32).
        let f = build_attn_prefill_fixture(4, 2, 32, 200, 900);
        assert!(
            f.max_seq > 34 * 32,
            "long-KV fixture must span >34 key tiles; got max_seq={}",
            f.max_seq
        );
        let got = run_gpu_attention_prefill(&ctx, &f);
    ...
    fn test_gpu_attention_prefill_seq_len_zero() {
        ...
        let mut enc = ctx.device.create_command_encoder(&Default::default());
        {
            ...
            pass.set_pipeline(&pipeline);
            pass.set_bind_group(0, &bg, &[]);
            pass.dispatch_workgroups(n_heads, n.div_ceil(8), 1);
        }
        ctx.submit_encoder(enc);
