...
use crate::quant::{BlockQ4_0, BlockQ4KM, BlockQ5K, BlockQ6K, BlockQ8_0};
...
pub(crate) mod neon {
    ...
    /// NEON-vectorized dot product of a Q5_K block with an f32 vector of length 256.
    pub unsafe fn vec_dot_q5_k_f32_neon(block: &BlockQ5K, y: &[f32]) -> f32 {
        unsafe {
            let d = crate::quant::f16_to_f32(block.d);
            let dmin = crate::quant::f16_to_f32(block.dmin);
            let (sc, mn) = crate::quant::decode_q4km_scales(&block.scales);
            let qs = &block.qs;
            let qh = &block.qh;
            let y_ptr = y.as_ptr();
            ...
            for j in 0..4 {
                ...
                for l in (0..32).step_by(4) {
                    ...
                    let hi1_0 = if h0 & u1 != 0 { 16.0f32 } else { 0.0f32 };
                    let hi1_1 = if h1 & u1 != 0 { 16.0f32 } else { 0.0f32 };
                    let hi1_2 = if h2 & u1 != 0 { 16.0f32 } else { 0.0f32 };
                    let hi1_3 = if h3 & u1 != 0 { 16.0f32 } else { 0.0f32 };

                    let hi2_0 = if h0 & u2 != 0 { 16.0f32 } else { 0.0f32 };
                    let hi2_1 = if h1 & u2 != 0 { 16.0f32 } else { 0.0f32 };
                    let hi2_2 = if h2 & u2 != 0 { 16.0f32 } else { 0.0f32 };
                    let hi2_3 = if h3 & u2 != 0 { 16.0f32 } else { 0.0f32 };
    ...
    /// NEON-vectorized dot product of a Q6_K block with an f32 vector of length 256.
    pub unsafe fn vec_dot_q6_k_f32_neon(block: &BlockQ6K, y: &[f32]) -> f32 {
        unsafe {
            let d = crate::quant::f16_to_f32(block.d);
            let ql = &block.ql;
            let qh = &block.qh;
            ...
            for _n in 0..2 {
                for is in 0..2 {
                    let sc1 = sc[sc_off + is] as f32;
                    let sc2 = sc[sc_off + is + 2] as f32;
                    let sc3 = sc[sc_off + is + 4] as f32;
                    let sc4 = sc[sc_off + is + 6] as f32;

                    let mut sum1v = vdupq_n_f32(0.0);
                    let mut sum2v = vdupq_n_f32(0.0);
                    let mut sum3v = vdupq_n_f32(0.0);
                    let mut sum4v = vdupq_n_f32(0.0);

                    let base_l = is * 16;
                    for l_idx in (0..16).step_by(4) {
                        let l = base_l + l_idx;
                        let mut q1_arr = [0.0f32; 4];
                        let mut q2_arr = [0.0f32; 4];
                        let mut q3_arr = [0.0f32; 4];
                        let mut q4_arr = [0.0f32; 4];

                        for k in 0..4 {
                            let cur_l = l + k;
                            let ql_val = ql[ql_off + cur_l];
                            let ql_val32 = ql[ql_off + cur_l + 32];
                            let qh_val = qh[qh_off + cur_l];

                            q1_arr[k] = (((ql_val & 0xF) | ((qh_val & 3) << 4)) as i8 - 32) as f32;
                            q2_arr[k] =
                                (((ql_val32 & 0xF) | (((qh_val >> 2) & 3) << 4)) as i8 - 32) as f32;
                            q3_arr[k] =
                                (((ql_val >> 4) | (((qh_val >> 4) & 3) << 4)) as i8 - 32) as f32;
                            q4_arr[k] =
                                (((ql_val32 >> 4) | (((qh_val >> 6) & 3) << 4)) as i8 - 32) as f32;
                        }

                        let q1_v = vld1q_f32(q1_arr.as_ptr());
                        let q2_v = vld1q_f32(q2_arr.as_ptr());
                        let q3_v = vld1q_f32(q3_arr.as_ptr());
                        let q4_v = vld1q_f32(q4_arr.as_ptr());

                        let y1 = vld1q_f32(y_ptr.add(y_off + l));
                        let y2 = vld1q_f32(y_ptr.add(y_off + l + 32));
                        let y3 = vld1q_f32(y_ptr.add(y_off + l + 64));
                        let y4 = vld1q_f32(y_ptr.add(y_off + l + 96));

                        sum1v = vfmaq_f32(sum1v, q1_v, y1);
                        sum2v = vfmaq_f32(sum2v, q2_v, y2);
                        sum3v = vfmaq_f32(sum3v, q3_v, y3);
                        sum4v = vfmaq_f32(sum4v, q4_v, y4);
                    }

                    sumf += sc1 * vaddvq_f32(sum1v)
                        + sc2 * vaddvq_f32(sum2v)
                        + sc3 * vaddvq_f32(sum3v)
                        + sc4 * vaddvq_f32(sum4v);
                }
    ...
    pub unsafe fn gemv_q4_0_q8_0_fused2_neon(
        ...
        let compute_slice = move |(start_row, chunk1): (usize, &mut [f32])| unsafe {
            ...
            // One row at a time: a 2-row interleave here would hold 8 f32
            // accumulators plus 16 live nibble vectors and spill NEON regs
            // to stack. The two matrices already supply the ILP; do not
            // unroll.
            while r < num_rows {
    ...
    /// Fused Gate + Up GEMV with in-register SwiGLU:
    ...
    pub unsafe fn gemv_q4_0_gate_up_swiglu_neon(
        ...
        let compute_slice = move |(start_row, chunk): (usize, &mut [f32])| unsafe {
            ...
            while r < num_rows {
                ...
                let mut gsum_even = vdupq_n_f32(0.0);
                let mut gsum_odd = vdupq_n_f32(0.0);
                let mut usum_even = vdupq_n_f32(0.0);
                let mut usum_odd = vdupq_n_f32(0.0);

                let mut bi = 0usize;
                while bi + 1 < blocks_per_row {
                    let gb0 = &*(ptrs.g().add(row0_start + bi * size_of::<BlockQ4_0>())
                        as *const BlockQ4_0);
                    let gb1 = &*(ptrs.g().add(row0_start + (bi + 1) * size_of::<BlockQ4_0>())
                        as *const BlockQ4_0);
                    let ub0 = &*(ptrs.u().add(row0_start + bi * size_of::<BlockQ4_0>())
                        as *const BlockQ4_0);
                    let ub1 = &*(ptrs.u().add(row0_start + (bi + 1) * size_of::<BlockQ4_0>())
                        as *const BlockQ4_0);

                    core::arch::asm!(
                        "prfm pldl1keep, [{0}, #256]",
                        "prfm pldl1keep, [{1}, #256]",
                        in(reg) (gb0 as *const BlockQ4_0),
                        in(reg) (ub0 as *const BlockQ4_0),
                        options(nostack, preserves_flags)
                    );
                    ...
                    let gv0 = vld1q_u8(gb0.qs.as_ptr());
                    let gv1 = vld1q_u8(gb1.qs.as_ptr());
                    let uv0 = vld1q_u8(ub0.qs.as_ptr());
                    let uv1 = vld1q_u8(ub1.qs.as_ptr());

                    let gv0_lo = vsubq_s8(vreinterpretq_s8_u8(vandq_u8(gv0, mask_lo)), offset_8);
                    let gv0_hi = vsubq_s8(vreinterpretq_s8_u8(vshrq_n_u8::<4>(gv0)), offset_8);
                    let gv1_lo = vsubq_s8(vreinterpretq_s8_u8(vandq_u8(gv1, mask_lo)), offset_8);
                    let gv1_hi = vsubq_s8(vreinterpretq_s8_u8(vshrq_n_u8::<4>(gv1)), offset_8);

                    let uv0_lo = vsubq_s8(vreinterpretq_s8_u8(vandq_u8(uv0, mask_lo)), offset_8);
                    let uv0_hi = vsubq_s8(vreinterpretq_s8_u8(vshrq_n_u8::<4>(uv0)), offset_8);
                    let uv1_lo = vsubq_s8(vreinterpretq_s8_u8(vandq_u8(uv1, mask_lo)), offset_8);
                    let uv1_hi = vsubq_s8(vreinterpretq_s8_u8(vshrq_n_u8::<4>(uv1)), offset_8);

                    let gp0 = vdotq_s32(vdotq_s32(z, gv0_lo, y0_lo), gv0_hi, y0_hi);
                    let gp1 = vdotq_s32(vdotq_s32(z, gv1_lo, y1_lo), gv1_hi, y1_hi);
                    let up0 = vdotq_s32(vdotq_s32(z, uv0_lo, y0_lo), uv0_hi, y0_hi);
                    let up1 = vdotq_s32(vdotq_s32(z, uv1_lo, y1_lo), uv1_hi, y1_hi);

                    let d_g0 = crate::quant::f16_to_f32(gb0.d) * xs0;
                    let d_g1 = crate::quant::f16_to_f32(gb1.d) * xs1;
                    let d_u0 = crate::quant::f16_to_f32(ub0.d) * xs0;
                    let d_u1 = crate::quant::f16_to_f32(ub1.d) * xs1;

                    gsum_even = vmlaq_n_f32(gsum_even, vcvtq_f32_s32(gp0), d_g0);
                    gsum_odd = vmlaq_n_f32(gsum_odd, vcvtq_f32_s32(gp1), d_g1);
                    usum_even = vmlaq_n_f32(usum_even, vcvtq_f32_s32(up0), d_u0);
                    usum_odd = vmlaq_n_f32(usum_odd, vcvtq_f32_s32(up1), d_u1);

                    bi += 2;
                }

                if bi < blocks_per_row {
                    let gb = &*(ptrs.g().add(row0_start + bi * size_of::<BlockQ4_0>())
                        as *const BlockQ4_0);
                    let ub = &*(ptrs.u().add(row0_start + bi * size_of::<BlockQ4_0>())
                        as *const BlockQ4_0);
                    ...
                    let d_g = crate::quant::f16_to_f32(gb.d) * xs;
                    let d_u = crate::quant::f16_to_f32(ub.d) * xs;

                    gsum_even = vmlaq_n_f32(gsum_even, vcvtq_f32_s32(gp), d_g);
                    usum_even = vmlaq_n_f32(usum_even, vcvtq_f32_s32(up), d_u);
                }

                let g = vaddvq_f32(gsum_even) + vaddvq_f32(gsum_odd);
                let u = vaddvq_f32(usum_even) + vaddvq_f32(usum_odd);
    ...
    /// Fused Q4_K gate + up GEMV + SwiGLU:
    ...
    pub unsafe fn gemv_q4k_gate_up_swiglu_neon(
        ...
        impl FusedQ4kPtrs {
            ...
            #[inline(always)]
            fn xqs(&self) -> *const i32 {
                self.xqs as *const i32
            }
        }

        let ptrs = FusedQ4kPtrs {
            g: gate_quant.as_ptr() as usize,
            u: up_quant.as_ptr() as usize,
            xq: x_quants.as_ptr() as usize,
            xs: x_scales.as_ptr() as usize,
    ...
    /// Unified Q5_K SwiGLU kernel with pre-quantized Q8_0 activations.
    ///
    ...
    #[allow(clippy::too_many_arguments)]
    pub unsafe fn gemv_q5k_q8_0_concat3_neon(
        a1_quant: &[u8],
        a2_quant: &[u8],
        a3_quant: &[u8],
        x_scales: &[f32],
        ...
        if cpu_features().tier >= CpuTier::NeonDotprod {
            unsafe {
                gemv_q5k_q8_0_concat3_neon_dotprod(
    ...
    /// Evaluates 3-matrix Q5_K GEMV using ARM NEON dotprod instructions.
    #[allow(clippy::too_many_arguments)]
    #[target_feature(enable = "neon,dotprod")]
    unsafe fn gemv_q5k_q8_0_concat3_neon_dotprod(
        a1_quant: &[u8],
        a2_quant: &[u8],
        a3_quant: &[u8],
        x_scales: &[f32],
        ...
        let compute_slice = move |start_row: usize, num_rows: usize| unsafe {
            ...
            while r + 1 < num_rows {
                ...
                for bi in 0..blocks_per_row {
                    let blk0 = &*(a_row0.add(bi * size_of::<BlockQ5K>()) as *const BlockQ5K);
                    let blk1 = &*(a_row1.add(bi * size_of::<BlockQ5K>()) as *const BlockQ5K);

                    core::arch::asm!(
                        "prfm pldl1keep, [{0}, #256]",
                        "prfm pldl1keep, [{1}, #256]",
                        in(reg) (blk0 as *const BlockQ5K),
                        in(reg) (blk1 as *const BlockQ5K),
                        options(nostack, preserves_flags)
                    );
                    ...
                    let mut qh0_0 = vld1q_u8(blk0.qh.as_ptr());
                    let mut qh0_1 = vld1q_u8(blk0.qh.as_ptr().add(16));
                    let mut qh1_0 = vld1q_u8(blk1.qh.as_ptr());
                    let mut qh1_1 = vld1q_u8(blk1.qh.as_ptr().add(16));
                    let xq_off = bi * 256;
                    ...
                    for j in 0..4 {
                        ...
                        let h0_lo_0 = vshlq_n_u8::<4>(vandq_u8(mone, qh0_0));
                        let h0_lo_1 = vshlq_n_u8::<4>(vandq_u8(mone, qh0_1));
                        let h0_hi_0 = vshlq_n_u8::<3>(vandq_u8(mtwo, qh0_0));
                        let h0_hi_1 = vshlq_n_u8::<3>(vandq_u8(mtwo, qh0_1));
                        qh0_0 = vshrq_n_u8::<2>(qh0_0);
                        qh0_1 = vshrq_n_u8::<2>(qh0_1);

                        let wlo0_0 =
                            vreinterpretq_s8_u8(vorrq_u8(vandq_u8(qb0_0, mask_0f), h0_lo_0));
                        let wlo0_1 =
                            vreinterpretq_s8_u8(vorrq_u8(vandq_u8(qb0_1, mask_0f), h0_lo_1));
                        let whi0_0 = vreinterpretq_s8_u8(vorrq_u8(vshrq_n_u8::<4>(qb0_0), h0_hi_0));
                        let whi0_1 = vreinterpretq_s8_u8(vorrq_u8(vshrq_n_u8::<4>(qb0_1), h0_hi_1));
                        ...
                        let h1_lo_0 = vshlq_n_u8::<4>(vandq_u8(mone, qh1_0));
                        let h1_lo_1 = vshlq_n_u8::<4>(vandq_u8(mone, qh1_1));
                        let h1_hi_0 = vshlq_n_u8::<3>(vandq_u8(mtwo, qh1_0));
                        let h1_hi_1 = vshlq_n_u8::<3>(vandq_u8(mtwo, qh1_1));
                        qh1_0 = vshrq_n_u8::<2>(qh1_0);
                        qh1_1 = vshrq_n_u8::<2>(qh1_1);

                        let wlo1_0 =
                            vreinterpretq_s8_u8(vorrq_u8(vandq_u8(qb1_0, mask_0f), h1_lo_0));
                        let wlo1_1 =
                            vreinterpretq_s8_u8(vorrq_u8(vandq_u8(qb1_1, mask_0f), h1_lo_1));
                        let whi1_0 = vreinterpretq_s8_u8(vorrq_u8(vshrq_n_u8::<4>(qb1_0), h1_hi_0));
                        let whi1_1 = vreinterpretq_s8_u8(vorrq_u8(vshrq_n_u8::<4>(qb1_1), h1_hi_1));
            ...
            while r < num_rows {
                ...
                for bi in 0..blocks_per_row {
                    ...
                    let (sc0, mn0) = crate::quant::decode_q4km_scales(&blk0.scales);
                    let qs0 = blk0.qs.as_ptr();
                    let mut qh0 = vld1q_u8(blk0.qh.as_ptr());
                    let mut qh1 = vld1q_u8(blk0.qh.as_ptr().add(16));
                    let xq_off = bi * 256;
                    ...
                    for j in 0..4 {
                        ...
                        let wlo0_0 = vreinterpretq_s8_u8(vorrq_u8(vandq_u8(qb0_0, mask_0f), h0_lo));
                        let wlo0_1 = vreinterpretq_s8_u8(vorrq_u8(vandq_u8(qb0_1, mask_0f), h1_lo));
                        let whi0_0 = vreinterpretq_s8_u8(vorrq_u8(vshrq_n_u8::<4>(qb0_0), h0_hi));
                        let whi0_1 = vreinterpretq_s8_u8(vorrq_u8(vshrq_n_u8::<4>(qb0_1), h1_hi));
        ...
        unsafe {
            ...
            let compute_slice = move |(start_row, chunk): (usize, &mut [f32])| unsafe {
                ...
                while r + 1 < num_rows {
                    ...
                    let row0_start = r0 * row_bytes;
                    let row1_start = r1 * row_bytes;
                    let mut sum0_v0 = vdupq_n_f32(0.0);
                    let mut sum0_v1 = vdupq_n_f32(0.0);
                    let mut sum1_v0 = vdupq_n_f32(0.0);
                    let mut sum1_v1 = vdupq_n_f32(0.0);

                    for bi in (0..n_blocks).step_by(2) {
                        let wb0_0 = &*(ptrs.a().add(row0_start + bi * size_of::<BlockQ8_0>())
                            as *const BlockQ8_0);
                        let wb1_0 = &*(ptrs.a().add(row1_start + bi * size_of::<BlockQ8_0>())
                            as *const BlockQ8_0);

                        let x0_lo = vld1q_s8(ptrs.xq().add(bi * 32));
                        let x0_hi = vld1q_s8(ptrs.xq().add(bi * 32 + 16));

                        let w0_0_lo = vld1q_s8(wb0_0.quants.as_ptr());
                        let w0_0_hi = vld1q_s8(wb0_0.quants.as_ptr().add(16));
                        let w1_0_lo = vld1q_s8(wb1_0.quants.as_ptr());
                        let w1_0_hi = vld1q_s8(wb1_0.quants.as_ptr().add(16));

                        let z = vdupq_n_s32(0);
                        let p0_0 = vdotq_s32(vdotq_s32(z, w0_0_lo, x0_lo), w0_0_hi, x0_hi);
                        let p1_0 = vdotq_s32(vdotq_s32(z, w1_0_lo, x0_lo), w1_0_hi, x0_hi);

                        let xs0 = *ptrs.xs().add(bi);
                        let d0_0 = crate::quant::f16_to_f32(wb0_0.delta) * xs0;
                        let d1_0 = crate::quant::f16_to_f32(wb1_0.delta) * xs0;

                        sum0_v0 = vmlaq_n_f32(sum0_v0, vcvtq_f32_s32(p0_0), d0_0);
                        sum1_v0 = vmlaq_n_f32(sum1_v0, vcvtq_f32_s32(p1_0), d1_0);

                        if bi + 1 < n_blocks {
                            let wb0_1 =
                                &*(ptrs.a().add(row0_start + (bi + 1) * size_of::<BlockQ8_0>())
                                    as *const BlockQ8_0);
                            let wb1_1 =
                                &*(ptrs.a().add(row1_start + (bi + 1) * size_of::<BlockQ8_0>())
                                    as *const BlockQ8_0);

                            let x1_lo = vld1q_s8(ptrs.xq().add((bi + 1) * 32));
                            let x1_hi = vld1q_s8(ptrs.xq().add((bi + 1) * 32 + 16));

                            let w0_1_lo = vld1q_s8(wb0_1.quants.as_ptr());
                            let w0_1_hi = vld1q_s8(wb0_1.quants.as_ptr().add(16));
                            let w1_1_lo = vld1q_s8(wb1_1.quants.as_ptr());
                            let w1_1_hi = vld1q_s8(wb1_1.quants.as_ptr().add(16));

                            let p0_1 = vdotq_s32(vdotq_s32(z, w0_1_lo, x1_lo), w0_1_hi, x1_hi);
                            let p1_1 = vdotq_s32(vdotq_s32(z, w1_1_lo, x1_lo), w1_1_hi, x1_hi);

                            let xs1 = *ptrs.xs().add(bi + 1);
                            let d0_1 = crate::quant::f16_to_f32(wb0_1.delta) * xs1;
                            let d1_1 = crate::quant::f16_to_f32(wb1_1.delta) * xs1;

                            sum0_v1 = vmlaq_n_f32(sum0_v1, vcvtq_f32_s32(p0_1), d0_1);
                            sum1_v1 = vmlaq_n_f32(sum1_v1, vcvtq_f32_s32(p1_1), d1_1);
                        }
                    }

                    chunk[r] = vaddvq_f32(sum0_v0) + vaddvq_f32(sum0_v1);
                    chunk[r + 1] = vaddvq_f32(sum1_v0) + vaddvq_f32(sum1_v1);
                    r += 2;
                }
                ...
                if r < num_rows {
                    ...
                    let z = vdupq_n_s32(0);
                    for bi in (0..n_blocks).step_by(2) {
                        let wb0 = &*(ptrs.a().add(row0_start + bi * size_of::<BlockQ8_0>())
                            as *const BlockQ8_0);
                        let x0_lo = vld1q_s8(ptrs.xq().add(bi * 32));
                        let x0_hi = vld1q_s8(ptrs.xq().add(bi * 32 + 16));
                        ...
                        sum0_v0 = vmlaq_n_f32(sum0_v0, vcvtq_f32_s32(p_0), d0);
                        ...
                        if bi + 1 < n_blocks {
                            ...
                            sum0_v1 = vmlaq_n_f32(sum0_v1, vcvtq_f32_s32(p_1), d1);
                        }
                    }
                    chunk[r] = vaddvq_f32(sum0_v0) + vaddvq_f32(sum0_v1);
                }
    ...
    pub unsafe fn gemv_q6k_q8_0_argmax_neon(
        ...
        #[cfg(all(feature = "parallel", not(target_arch = "wasm32")))]
        crate::backend::cpu::par_range(m, chunk, |start_row, num_rows| {
            compute_chunk(start_row, num_rows);
        });
        ...
        for &(v, idx) in worker_results.iter() {
            if v > best_val {
                best_val = v;
                best_idx = idx;
            }
    ...
    unsafe fn gemv_q4k_q8_0_neon_dotprod(
        ...
        unsafe {
            ...
            let mut heap_xqs;
            let xqs: &[i32] = if nb32 <= 1024 {
                q8_0_col_sums_slice(x_quants, 1, k, &mut stack_xqs[..nb32]);
                &stack_xqs[..nb32]
            } else {
                heap_xqs = vec![0i32; nb32];
                q8_0_col_sums_slice(x_quants, 1, k, &mut heap_xqs);
                &heap_xqs
            };
    ...
    /// NEON Q5_K × Q8_0 integer GEMV with pre-quantized input using dotprod.
    ///
    /// Extracts 5-bit quants as unsigned i8, dots with Q8_0 input using `vdotq_s32`,
    /// and scales by sub-block factors while applying precomputed activation column sums.
    #[target_feature(enable = "neon,dotprod")]
    unsafe fn gemv_q5k_q8_0_neon_dotprod(
        a_quant: &[u8],
        x_scales: &[f32],
        x_quants: &[i8],
        y: &mut [f32],
    ...
    #[target_feature(enable = "neon,dotprod")]
    unsafe fn q8_0_col_sums_slice(b_quants: &[i8], n: usize, k: usize, sums: &mut [i32]) {
        unsafe {
            let nb32 = k / 32;
    ...
    /// Compute column sums for Q8_0 blocks across `n` columns of dimension `k`.
    #[target_feature(enable = "neon,dotprod")]
    unsafe fn q8_0_col_sums(b_quants: &[i8], n: usize, k: usize) -> Vec<i32> {
        let nb32 = k / 32;
        let mut sums = vec![0i32; n * nb32];
        unsafe {
            q8_0_col_sums_slice(b_quants, n, k, &mut sums);
        }
        sums
    }
    ...
    unsafe fn gemv_q4_1_q8_0_neon_dotprod(
        ...
        debug_assert!(
            x_scales.len() >= k / 32 && x_quants.len() >= k,
            "Q4_1 GEMV: activation scratch too small"
        );
        unsafe {
            let blocks_per_row = k / 32;
            ...
            debug_assert_eq!(
                a_quant.len(),
                _m * row_bytes,
                "Q4_1 GEMV: a_quant size mismatch"
            );
            let a_base = a_quant.as_ptr() as usize;
            let xq_base = x_quants.as_ptr() as usize;
            let xs_base = x_scales.as_ptr() as usize;
            ...
            let mut stack_xqs = [0i32; 1024];
            let mut heap_xqs;
            let xqs: &[i32] = if nb <= 1024 {
                q8_0_col_sums_slice(x_quants, 1, k, &mut stack_xqs[..nb]);
                &stack_xqs[..nb]
            } else {
                heap_xqs = vec![0i32; nb];
                q8_0_col_sums_slice(x_quants, 1, k, &mut heap_xqs);
                &heap_xqs
            };
            ...
            let compute_slice = move |(start_row, chunk): (usize, &mut [f32])| unsafe {
                ...
                while r + 3 < num_rows {
                    ...
                    let mut sumf3 = 0.0f32;

                    for bi in 0..blocks_per_row {
                        let blk0 = &*((a_base + row0_start + bi * size_of::<BlockQ4_1>())
                            as *const BlockQ4_1);
                        let blk1 = &*((a_base + row1_start + bi * size_of::<BlockQ4_1>())
                            as *const BlockQ4_1);
                        let blk2 = &*((a_base + row2_start + bi * size_of::<BlockQ4_1>())
                            as *const BlockQ4_1);
                        let blk3 = &*((a_base + row3_start + bi * size_of::<BlockQ4_1>())
                            as *const BlockQ4_1);

                        let d0 = crate::quant::f16_to_f32(blk0.d);
                        let m0 = crate::quant::f16_to_f32(blk0.m);
                        let d1 = crate::quant::f16_to_f32(blk1.d);
                        let m1 = crate::quant::f16_to_f32(blk1.m);
                        let d2 = crate::quant::f16_to_f32(blk2.d);
                        let m2 = crate::quant::f16_to_f32(blk2.m);
                        let d3 = crate::quant::f16_to_f32(blk3.d);
                        let m3 = crate::quant::f16_to_f32(blk3.m);

                        let xp = (xq_base as *const i8).add(bi * 32);
                        let x0 = vld1q_s8(xp);
                        let x1 = vld1q_s8(xp.add(16));

                        let xs_val = *(xs_base as *const f32).add(bi);
                        let sx_val = *xqs.get_unchecked(bi);

                        // Row 0
                        let qb0 = vld1q_u8(blk0.qs.as_ptr());
                        let w_lo0 = vreinterpretq_s8_u8(vandq_u8(qb0, mask_0f));
                        let w_hi0 = vreinterpretq_s8_u8(vshrq_n_u8::<4>(qb0));
                        let dp0 = vaddvq_s32(vdotq_s32(vdotq_s32(z, w_lo0, x0), w_hi0, x1));
                        sumf0 += xs_val * (d0 * dp0 as f32 + m0 * sx_val as f32);

                        // Row 1
                        let qb1 = vld1q_u8(blk1.qs.as_ptr());
                        let w_lo1 = vreinterpretq_s8_u8(vandq_u8(qb1, mask_0f));
                        let w_hi1 = vreinterpretq_s8_u8(vshrq_n_u8::<4>(qb1));
                        let dp1 = vaddvq_s32(vdotq_s32(vdotq_s32(z, w_lo1, x0), w_hi1, x1));
                        sumf1 += xs_val * (d1 * dp1 as f32 + m1 * sx_val as f32);

                        // Row 2
                        let qb2 = vld1q_u8(blk2.qs.as_ptr());
                        let w_lo2 = vreinterpretq_s8_u8(vandq_u8(qb2, mask_0f));
                        let w_hi2 = vreinterpretq_s8_u8(vshrq_n_u8::<4>(qb2));
                        let dp2 = vaddvq_s32(vdotq_s32(vdotq_s32(z, w_lo2, x0), w_hi2, x1));
                        sumf2 += xs_val * (d2 * dp2 as f32 + m2 * sx_val as f32);

                        // Row 3
                        let qb3 = vld1q_u8(blk3.qs.as_ptr());
                        let w_lo3 = vreinterpretq_s8_u8(vandq_u8(qb3, mask_0f));
                        let w_hi3 = vreinterpretq_s8_u8(vshrq_n_u8::<4>(qb3));
                        let dp3 = vaddvq_s32(vdotq_s32(vdotq_s32(z, w_lo3, x0), w_hi3, x1));
                        sumf3 += xs_val * (d3 * dp3 as f32 + m3 * sx_val as f32);
                    }
                ...
                while r < num_rows {
                    ...
                    for bi in 0..blocks_per_row {
                        let blk = &*((a_base + row_start + bi * size_of::<BlockQ4_1>())
                            as *const BlockQ4_1);
                        let d = crate::quant::f16_to_f32(blk.d);
                        let m = crate::quant::f16_to_f32(blk.m);
                        ...
                        sumf += xs_val * (d * dp as f32 + m * sx_val as f32);
                    }
    ...
    /// B is column-major Q8_0 activations, and C is column-major output.
    #[target_feature(enable = "neon,dotprod")]
    ...
    /// NEON Q5_K GEMV: y[m] = A_q5_k[m,k] @ x_f32[k].
    pub unsafe fn gemv_q5k_f32_neon(
        a_quant: &[u8],
        x: &[f32],
        y: &mut [f32],
        m: usize,
    ...
    /// Q5_K pre-quantized GEMV dispatcher (input already Q8_0).
    pub unsafe fn gemv_q5k_q8_0_neon(
        a_quant: &[u8],
        x_scales: &[f32],
        x_quants: &[i8],
        y: &mut [f32],
    ...
    /// Q4_K pre-quantized GEMV dispatcher (input already Q8_0).
    pub unsafe fn gemv_q4k_q8_0_neon(
        a_quant: &[u8],
        x_scales: &[f32],
        x_quants: &[i8],
        y: &mut [f32],
    ...
    mod fallback_tests {
        ...
        fn random_q5km(st: &mut u64) -> BlockQ5K {
            let mut scales = [0u8; 12];
            let mut qs = [0u8; 128];
            let mut qh = [0u8; 32];
            for b in scales.iter_mut() {
        ...
        #[test]
        fn q4k_gate_up_swiglu_matches_separate_gemv() {
            if !require_simd_or_skip("dotprod", cpu_features().dotprod) {
                return;
            }
            // 2 super-blocks/row (k=512) * 7 rows: exercises multi-block accumulation,
            // 2-row unrolling (3 pairs), and single remainder row.
            let (m, k, nb) = (7usize, 512usize, 2usize);
            let mut st = 0xbeef_cafeu64;
            let g_blocks: Vec<BlockQ4KM> = (0..m * nb).map(|_| random_q4km(&mut st)).collect();
            let u_blocks: Vec<BlockQ4KM> = (0..m * nb).map(|_| random_q4km(&mut st)).collect();
            let g_bytes = blocks_to_bytes(&g_blocks);
            let u_bytes = blocks_to_bytes(&u_blocks);
            let x: Vec<f32> = (0..k).map(|_| lcg(&mut st)).collect();
            let (xs, xq) = quantize_col(&x);
            ...
            let mut y_up = vec![0.0f32; m];
            unsafe {
                gemv_q4k_q8_0_neon_dotprod(&g_bytes, &xs, &xq, &mut y_gate, m, k);
                gemv_q4k_q8_0_neon_dotprod(&u_bytes, &xs, &xq, &mut y_up, m, k);
            }
            crate::backend::cpu::silu_mul_inplace(&mut y_gate, &y_up);

            for i in 0..m {
                let diff = (out_fused[i] - y_gate[i]).abs();
                assert!(
                    diff < 1e-4,
        ...
        #[test]
        fn q4_0_gate_up_swiglu_matches_separate_gemv() {
            if !require_simd_or_skip("dotprod", cpu_features().dotprod) {
                return;
            }
            // 4 blocks/row (k=128) x 7 rows: multi-block accumulation plus an
            // odd row count (the fused loop is single-row; every row takes the
            // same path, so this also covers chunk-boundary rows via the pool).
            let (m, k, nb) = (7usize, 128usize, 4usize);
            let mut st = 0xcafe_f00du64;
            let mut random_blocks = || {
                let blocks: Vec<BlockQ4_0> = (0..m * nb)
                    .map(|_| {
                        let mut qs = [0u8; 16];
                        for b in qs.iter_mut() {
            ...
            let g_bytes = random_blocks();
            let u_bytes = random_blocks();
            let x: Vec<f32> = (0..k).map(|_| lcg(&mut st)).collect();
            let (xs, xq) = quantize_col(&x);

            let mut out_fused = vec![0.0f32; m];
            unsafe {
                gemv_q4_0_gate_up_swiglu_neon(&g_bytes, &u_bytes, &xs, &xq, &mut out_fused, m, k);
            }

            let mut y_gate = vec![0.0f32; m];
            let mut y_up = vec![0.0f32; m];
            unsafe {
                gemv_q4_0_q8_0_neon_dotprod(&g_bytes, &xs, &xq, &mut y_gate, m, k);
                gemv_q4_0_q8_0_neon_dotprod(&u_bytes, &xs, &xq, &mut y_up, m, k);
            }
            crate::backend::cpu::silu_mul_inplace(&mut y_gate, &y_up);
        ...
        #[test]
        fn q4_0_fused2_matches_separate_gemv() {
            if !require_simd_or_skip("dotprod", cpu_features().dotprod) {
                return;
            }
            let (m, k, nb) = (7usize, 128usize, 4usize);
            ...
            let a1_bytes = random_blocks();
            let a2_bytes = random_blocks();
            let x: Vec<f32> = (0..k).map(|_| lcg(&mut st)).collect();
            let (xs, xq) = quantize_col(&x);

            let mut y1_fused = vec![0.0f32; m];
            let mut y2_fused = vec![0.0f32; m];
            unsafe {
                gemv_q4_0_q8_0_fused2_neon(
                    &a1_bytes,
                    &a2_bytes,
                    &xs,
                    &xq,
            ...
            let mut y1_ref = vec![0.0f32; m];
            let mut y2_ref = vec![0.0f32; m];
            unsafe {
                gemv_q4_0_q8_0_neon_dotprod(&a1_bytes, &xs, &xq, &mut y1_ref, m, k);
                gemv_q4_0_q8_0_neon_dotprod(&a2_bytes, &xs, &xq, &mut y2_ref, m, k);
            }

            for i in 0..m {
                let d1 = (y1_fused[i] - y1_ref[i]).abs();
                let d2 = (y2_fused[i] - y2_ref[i]).abs();
                assert!(
                    d1 < 1e-4 && d2 < 1e-4,
                    "row {i}: fused ({}, {}) vs separate ({}, {})",
                    y1_fused[i],
                    y2_fused[i],
                    y1_ref[i],
                    y2_ref[i]
        ...
        #[test]
        fn q5k_gemv_dotprod_matches_scalar() {
            if !require_simd_or_skip("dotprod", cpu_features().dotprod) {
                return;
            }
            let (m, k, nb) = (5usize, 512usize, 2usize);
            let mut st = 0xcafe_f00du64;
            let blocks: Vec<BlockQ5K> = (0..m * nb).map(|_| random_q5km(&mut st)).collect();
            let a = blocks_to_bytes(&blocks);
            let x: Vec<f32> = (0..k).map(|_| lcg(&mut st)).collect();
            let (xs, xq) = quantize_col(&x);

            let mut y_dot = vec![0.0f32; m];
            unsafe { gemv_q5k_q8_0_neon_dotprod(&a, &xs, &xq, &mut y_dot, m, k) };

            let xf = reconstruct_q8_0_input(&xs, &xq, k);
            let mut y_fb = vec![0.0f32; m];
            crate::backend::cpu::gemv_q5km_f32(&a, &xf, &mut y_fb, m, k);
            assert_close(&y_dot, &y_fb);
        }

        #[test]
        fn q5k_gemv_wrapper_matches_exact_f32() {
            if !require_simd_or_skip("dotprod", cpu_features().dotprod) {
                return;
            }
            let (m, k, nb) = (4usize, 256usize, 1usize);
            ...
            let mut q = Vec::new();
            let mut y_dot = vec![0.0f32; m];
            unsafe { gemv_q5k_f32_neon(&a, &x, &mut y_dot, m, k, &mut s, &mut q) };

            let mut y_exact = vec![0.0f32; m];
            crate::backend::cpu::gemv_q5km_f32(&a, &x, &mut y_exact, m, k);
        ...
        #[test]
        fn q5k_gate_up_swiglu_matches_separate_gemv() {
            if !require_simd_or_skip("dotprod", cpu_features().dotprod) {
                return;
            }
            let (m, k, nb) = (7usize, 512usize, 2usize);
            let mut st = 0xbeef_5555u64;
            let g_blocks: Vec<BlockQ5K> = (0..m * nb).map(|_| random_q5km(&mut st)).collect();
            let u_blocks: Vec<BlockQ5K> = (0..m * nb).map(|_| random_q5km(&mut st)).collect();
            let g_bytes = blocks_to_bytes(&g_blocks);
            let u_bytes = blocks_to_bytes(&u_blocks);
            let x: Vec<f32> = (0..k).map(|_| lcg(&mut st)).collect();
            let (xs, xq) = quantize_col(&x);
            ...
            let mut out_fused = vec![0.0f32; m];
            unsafe {
                gemv_q5k_gate_up_swiglu_neon(&g_bytes, &u_bytes, &xs, &xq, &mut out_fused, m, k);
            }

            let mut y_gate = vec![0.0f32; m];
            let mut y_up = vec![0.0f32; m];
            unsafe {
                gemv_q5k_q8_0_neon_dotprod(&g_bytes, &xs, &xq, &mut y_gate, m, k);
                gemv_q5k_q8_0_neon_dotprod(&u_bytes, &xs, &xq, &mut y_up, m, k);
            }
            crate::backend::cpu::silu_mul_inplace(&mut y_gate, &y_up);
        ...
        #[test]
        fn q5k_concat3_matches_separate_gemv() {
            if !require_simd_or_skip("dotprod", cpu_features().dotprod) {
                return;
            }
            let (m1, m2, m3, k) = (5usize, 3usize, 7usize, 512usize);
            let nb = k / 256;
            let mut st = 0x5555_1234u64;
            let a1_blocks: Vec<BlockQ5K> = (0..m1 * nb).map(|_| random_q5km(&mut st)).collect();
            let a2_blocks: Vec<BlockQ5K> = (0..m2 * nb).map(|_| random_q5km(&mut st)).collect();
            let a3_blocks: Vec<BlockQ5K> = (0..m3 * nb).map(|_| random_q5km(&mut st)).collect();
            let a1_bytes = blocks_to_bytes(&a1_blocks);
            let a2_bytes = blocks_to_bytes(&a2_blocks);
            let a3_bytes = blocks_to_bytes(&a3_blocks);
            let x: Vec<f32> = (0..k).map(|_| lcg(&mut st)).collect();
            ...
            let mut ref3 = vec![0.0f32; m3];
            unsafe {
                gemv_q5k_q8_0_neon(&a1_bytes, &xs, &xq, &mut ref1, m1, k);
                gemv_q5k_q8_0_neon(&a2_bytes, &xs, &xq, &mut ref2, m2, k);
                gemv_q5k_q8_0_neon(&a3_bytes, &xs, &xq, &mut ref3, m3, k);
            }
        ...
        #[test]
        fn q4_1_gemv_neon_matches_scalar() {
            let (m, k) = (7usize, 128usize);
            let nb = k / 32;
            let mut st = 0x0411_beefu64;
            let blocks: Vec<BlockQ4_1> = (0..m * nb)
                .map(|_| {
                    let mut qs = [0u8; 16];
                    for b in qs.iter_mut() {
            ...
            unsafe {
                gemm_q4_1_q8_0_neon(&a, &xs, &xq, &mut y_gemm, m, 1, k);
            }
...
mod avx2 {
    ...
    /// AVX2-vectorized dot product of a Q5_K block with an f32 vector of length 256.
    #[target_feature(enable = "avx2", enable = "fma")]
    pub unsafe fn vec_dot_q5_k_f32_avx2(block: &BlockQ5K, y: &[f32]) -> f32 {
        unsafe {
            let d = crate::quant::f16_to_f32(block.d);
            let dmin = crate::quant::f16_to_f32(block.dmin);
            let (sc, mn) = crate::quant::decode_q4km_scales(&block.scales);
            ...
            for j in 0..4 {
                ...
                for l in (0..32).step_by(8) {
                    let mut lo_arr = [0.0f32; 8];
                    let mut hi_arr = [0.0f32; 8];
                    for k in 0..8 {
                        let cur_l = l + k;
                        let q = qs[qi + cur_l];
                        let h = qh[cur_l];
                        let hi1 = if h & u1 != 0 { 16.0f32 } else { 0.0f32 };
                        let hi2 = if h & u2 != 0 { 16.0f32 } else { 0.0f32 };
                        lo_arr[k] = ((q & 0xF) as f32) + hi1;
                        hi_arr[k] = ((q >> 4) as f32) + hi2;
                    }

                    let lo_f32 = _mm256_loadu_ps(lo_arr.as_ptr());
                    let hi_f32 = _mm256_loadu_ps(hi_arr.as_ptr());
    ...
    /// AVX2-vectorized dot product of a Q6_K block with an f32 vector of length 256.
    #[target_feature(enable = "avx2", enable = "fma")]
    pub unsafe fn vec_dot_q6_k_f32_avx2(block: &BlockQ6K, y: &[f32]) -> f32 {
        unsafe {
            let d = crate::quant::f16_to_f32(block.d);
            let ql = &block.ql;
            let qh = &block.qh;
            ...
            for _n in 0..2 {
                for is in 0..2 {
                    ...
                    let mut sum1_acc = _mm256_setzero_ps();
                    let mut sum2_acc = _mm256_setzero_ps();
                    let mut sum3_acc = _mm256_setzero_ps();
                    let mut sum4_acc = _mm256_setzero_ps();

                    let base_l = is * 16;
                    for l_idx in (0..16).step_by(8) {
                        let l = base_l + l_idx;
                        let mut q1_arr = [0.0f32; 8];
                        let mut q2_arr = [0.0f32; 8];
                        let mut q3_arr = [0.0f32; 8];
                        let mut q4_arr = [0.0f32; 8];
                        ...
                        let q1_f32 = _mm256_loadu_ps(q1_arr.as_ptr());
                        let q2_f32 = _mm256_loadu_ps(q2_arr.as_ptr());
                        let q3_f32 = _mm256_loadu_ps(q3_arr.as_ptr());
                        let q4_f32 = _mm256_loadu_ps(q4_arr.as_ptr());

                        let y1 = _mm256_loadu_ps(y_ptr.add(y_off + l));
                        let y2 = _mm256_loadu_ps(y_ptr.add(y_off + l + 32));
                        let y3 = _mm256_loadu_ps(y_ptr.add(y_off + l + 64));
                        let y4 = _mm256_loadu_ps(y_ptr.add(y_off + l + 96));

                        sum1_acc = _mm256_fmadd_ps(q1_f32, y1, sum1_acc);
                        sum2_acc = _mm256_fmadd_ps(q2_f32, y2, sum2_acc);
                        sum3_acc = _mm256_fmadd_ps(q3_f32, y3, sum3_acc);
                        sum4_acc = _mm256_fmadd_ps(q4_f32, y4, sum4_acc);
                    }

                    sumf += sc1 * hsum_avx(sum1_acc)
                        + sc2 * hsum_avx(sum2_acc)
                        + sc3 * hsum_avx(sum3_acc)
                        + sc4 * hsum_avx(sum4_acc);
                }
...
/// Best available Q5_K dot product.
pub fn vec_dot_q5_k_f32(block: &BlockQ5K, y: &[f32]) -> f32 {
    assert_eq!(y.len(), 256, "Q5_K vec_dot requires y.len() == 256");
    #[cfg(target_arch = "aarch64")]
    {
        unsafe { neon::vec_dot_q5_k_f32_neon(block, y) }
    }
    ...
    #[cfg(target_arch = "x86_64")]
    {
        use crate::backend::cpu_features::{CpuTier, cpu_features};
        match cpu_features().tier {
            CpuTier::Scalar => crate::quant::vec_dot_q5_k_f32_scalar(block, y),
            _ => unsafe { avx2::vec_dot_q5_k_f32_avx2(block, y) },
        }
    }

    #[cfg(not(any(target_arch = "aarch64", target_arch = "x86_64")))]
    {
        crate::quant::vec_dot_q5_k_f32_scalar(block, y)
    }
}

/// Best available Q6_K dot product.
pub fn vec_dot_q6_k_f32(block: &BlockQ6K, y: &[f32]) -> f32 {
    assert_eq!(y.len(), 256, "Q6_K vec_dot requires y.len() == 256");
    #[cfg(target_arch = "aarch64")]
    {
        unsafe { neon::vec_dot_q6_k_f32_neon(block, y) }
    }
    ...
    #[cfg(target_arch = "x86_64")]
    {
        use crate::backend::cpu_features::{CpuTier, cpu_features};
        match cpu_features().tier {
            CpuTier::Scalar => crate::quant::vec_dot_q6_k_f32_scalar(block, y),
            _ => unsafe { avx2::vec_dot_q6_k_f32_avx2(block, y) },
        }
    }

    #[cfg(not(any(target_arch = "aarch64", target_arch = "x86_64")))]
    {
        crate::quant::vec_dot_q6_k_f32_scalar(block, y)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_simd_q5_k_matches_scalar() {
        ...
        let scalar = crate::quant::vec_dot_q5_k_f32_scalar(&block, &y);
        let simd = vec_dot_q5_k_f32(&block, &y);

        assert!(
            (scalar - simd).abs() < 1e-2,
            "SIMD Q5_K mismatch: scalar={scalar}, simd={simd}"
        );
    }

    #[test]
    fn test_simd_q6_k_matches_scalar() {
        let mut block = BlockQ6K {
            ql: [0; 128],
            qh: [0; 64],
            scales: [0; 16],
            d: f16::from_f32(0.25).to_bits(),
        };
        for (i, s) in block.scales.iter_mut().enumerate() {
            *s = ((i as i8) * 5) - 30;
        }
        for (i, b) in block.ql.iter_mut().enumerate() {
            *b = ((i % 11) as u8) | (((i % 7) as u8) << 4);
        }
        for (i, b) in block.qh.iter_mut().enumerate() {
            *b = (i * 19 + 2) as u8;
        }
        ...
        let scalar = crate::quant::vec_dot_q6_k_f32_scalar(&block, &y);
        let simd = vec_dot_q6_k_f32(&block, &y);

        assert!(
            (scalar - simd).abs() < 1e-2,
            "SIMD Q6_K mismatch: scalar={scalar}, simd={simd}"
        );
    }
