...
/// Scalar GeLU approximation matching `ggml_gelu` (tanh-form).
#[inline(always)]
...
pub fn axpy_inplace(a: &mut [f32], b: &[f32], alpha: f32) {
    assert_eq!(
        a.len(),
        b.len(),
        "axpy_inplace length mismatch: {} vs {}",
        a.len(),
        b.len()
    );
...
mod tests {
        ...
        for n in [1025, 2048, 2049, 3072] {
            ...
            for (i, (&got, &expected)) in gate_large.iter().zip(ref_large.iter()).enumerate() {
                assert!(
                    ...
                    "silu_mul large mismatch for n={n} at {i}: got {got}, expected {expected}"
                );
            }
        }
    }
    ...
    fn test_gelu_mul_inplace() {
        ...
        for n in [1025, 2048, 2049, 3072] {
            ...
            for (i, (&got, &expected)) in gate_large.iter().zip(ref_large.iter()).enumerate() {
                assert!(
                    (got - expected).abs() < 1e-5,
                    "gelu_mul large mismatch for n={n} at {i}: got {got}, expected {expected}"
                );
            }
        }

        // Test extreme value saturation
        assert_eq!(gelu_approx_f32(-50.0), 0.0);
        assert_eq!(gelu_approx_f32(50.0), 50.0);
    }
    ...
    fn test_softcap_inplace() {
        ...
        // Test non-positive or non-finite cap no-ops
        let original = vec![1.0, 2.0, 3.0];
        for invalid_cap in [0.0, -10.0, f32::NAN, f32::INFINITY, f32::NEG_INFINITY] {
            let mut test_vec = original.clone();
            softcap_inplace(&mut test_vec, invalid_cap);
            assert_eq!(
                test_vec, original,
                "softcap with cap={invalid_cap} must no-op"
            );
        }

        // Test empty slice safety
        let mut empty: Vec<f32> = vec![];
        softcap_inplace(&mut empty, cap);
        assert!(empty.is_empty());

        // Test extreme and subnormal values
        let mut extremes = vec![1e30, -1e30, 1e-38, -1e-38];
        softcap_inplace(&mut extremes, cap);
        assert!((extremes[0] - cap).abs() < 1e-5);
        assert!((extremes[1] - (-cap)).abs() < 1e-5);
        assert!(extremes[2].is_finite());
        assert!(extremes[3].is_finite());

        // Test serial and parallel branches (threshold >= 2048)
        for n in [1025, 1537, 2048, 2049, 3072] {
            let mut x_large: Vec<f32> = (0..n).map(|i| (i as f32 - 1024.0) * 0.1).collect();
            let expected_large: Vec<f32> =
                x_large.iter().map(|&v| cap * (v / cap).tanh()).collect();
            softcap_inplace(&mut x_large, cap);
            for (i, (&got, &exp)) in x_large.iter().zip(expected_large.iter()).enumerate() {
                assert!(
                    (got - exp).abs() < 1e-5,
                    "softcap large mismatch for n={n} at {i}: got {got}, expected {exp}"
                );
            }
        }
    }
    ...
    fn test_apply_rope_norm_yarn_to_head_empty_or_short() {
        ...
        let mut head_bad = vec![42.0; 16];
        apply_rope_norm_yarn_to_head(&mut head_bad, 1, 16, 0.0, &yarn);
        assert_eq!(head_bad, vec![42.0; 16]);

        let mut head_bad_yarn = vec![42.0; 16];
        apply_rope_norm_yarn_to_head(&mut head_bad_yarn, 1, 16, 10000.0, &yarn_bad);
        assert_eq!(head_bad_yarn, vec![42.0; 16]);
    }
    ...
    fn test_axpy_inplace() {
        ...
        // Non-finite alpha early return
        let mut a = vec![1.0, 2.0, 3.0];
        let b = vec![4.0, 5.0, 6.0];
        axpy_inplace(&mut a, &b, f32::NAN);
        assert_eq!(a, vec![1.0, 2.0, 3.0]);

        axpy_inplace(&mut a, &b, f32::INFINITY);
        assert_eq!(a, vec![1.0, 2.0, 3.0]);
    ...
    #[test]
    #[should_panic(expected = "axpy_inplace length mismatch")]
    fn test_axpy_inplace_length_mismatch_even_with_zero_alpha() {
    ...
    #[test]
    fn test_rmsnorm_truncated_weight_returns_early() {
        let mut x = vec![1.0, 2.0, 3.0, 4.0];
