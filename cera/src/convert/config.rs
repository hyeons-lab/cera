...
impl HfModelConfig {
    ...
    pub fn apply_to_gguf_writer(&self, writer: &mut GgufWriter, model_name: &str) {
        ...
        if (arch == "olmo" || arch == "olmo2" || arch == "phi3")
        ...
        if arch == "bailingmoe3" {
            ...
            if let Some(n_exp) = self.extra.get("num_experts").and_then(Value::as_u64) {
        ...
        if arch == "mistral3" {
            let rope_scaling = self.extra.get("rope_scaling");
            ...
            if let Some(log_mul) = self
                ...
                .or_else(|| rope_scaling.and_then(|v| v.get("yarn_log_multiplier")))
                .and_then(Value::as_f64)
            ...
            if let Some(beta_fast) = rope_scaling
                .and_then(|v| v.get("yarn_beta_fast").or_else(|| v.get("beta_fast")))
                .and_then(Value::as_f64)
            {
                writer.add_f32("mistral3.rope.scaling.yarn_beta_fast", beta_fast as f32);
            }
            if let Some(beta_slow) = rope_scaling
                .and_then(|v| v.get("yarn_beta_slow").or_else(|| v.get("beta_slow")))
                .and_then(Value::as_f64)
            {
