...
impl LlamaModel {
    ...
    pub fn from_gguf_with_id(
        ...
        let arch = gguf
            ...
            .to_lowercase();
        let prefix_str = match arch.as_str() {
            "phi"
                if !gguf.metadata.contains_key("phi.block_count")
                    && gguf.metadata.contains_key("phi3.block_count") =>
            {
                "phi3"
            }
            "ministral3"
                if !gguf.metadata.contains_key("ministral3.block_count")
                    && gguf.metadata.contains_key("mistral3.block_count") =>
            {
                "mistral3"
            }
            _ => arch.as_str(),
        ...
        let default_rope_theta = if matches!(prefix, "minicpm" | "minicpm5" | "phi3" | "phi") {
            10_000.0
        } else {
            1_000_000.0
        };
    ...
    fn forward_prefill_batched(
        ...
        for layer in 0..cfg.n_layers {
            ...
            if let Some(bias) = self.attn_output_bias[layer].as_deref() {
                if n == 1 {
                    let len = hs.min(bias.len());
                    cpu::add_inplace(&mut block_out[..len], &bias[..len]);
                } else {
            ...
            if let Some(bias) = self.ffn_gate_bias[layer].as_deref() {
                if n == 1 {
                    let len = is.min(bias.len());
                    cpu::add_inplace(&mut gate_mat[..len], &bias[..len]);
                } else {
            ...
            if let Some(bias) = self.ffn_up_bias[layer].as_deref() {
                if n == 1 {
                    let len = is.min(bias.len());
                    cpu::add_inplace(&mut up_mat[..len], &bias[..len]);
                } else {
                    for (i, &b) in bias.iter().enumerate().take(is) {
            ...
            if let Some(bias) = self.ffn_down_bias[layer].as_deref() {
                if n == 1 {
                    let len = hs.min(bias.len());
                    cpu::add_inplace(&mut ffn_out[..len], &bias[..len]);
                } else {
                    for (i, &b) in bias.iter().enumerate().take(hs) {
