...
pub(crate) fn forward_attn_block(
    ...
    if let Some(bias) = extras.attn_output_bias {
        cpu::add_inplace(&mut state.scratch.out[..hidden_size], bias);
    }
}
...
pub(crate) fn forward_ffn_block(
    ...
    let lora = state.lora.clone();
    #[allow(unused_mut)]
    let mut fused_swiglu_done = false;
    #[cfg(target_arch = "aarch64")]
    {
    ...
    if !fused_swiglu_done {
        if let Some(b) = extras.gate_bias {
            cpu::add_inplace(&mut state.scratch.gate[..intermediate_size], b);
        }
        if let Some(b) = extras.up_bias {
            cpu::add_inplace(&mut state.scratch.up[..intermediate_size], b);
        }
    ...
    if let Some(b) = extras.down_bias {
        cpu::add_inplace(&mut state.scratch.out[..hidden_size], b);
    }
}
