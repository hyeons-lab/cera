...
impl WeightRef {
    ...
    pub fn slice_rows(&self, row_start: usize, num_rows: usize) -> Result<Self> {
        ...
        let end_abs = start
            .checked_add(size as u64)
            .context("overflow computing absolute end byte for sliced rows")?;
        usize::try_from(start)
            .context("sliced weight start offset exceeds platform pointer width")?;
        usize::try_from(end_abs)
            .context("sliced weight end offset exceeds platform pointer width")?;
...
pub(crate) fn forward_attn_block(
    ...
    #[cfg(target_arch = "aarch64")]
    {
        let valid_dims = weights.attn_q.k == hidden.len()
            && weights.attn_k.k == hidden.len()
            && weights.attn_v.k == hidden.len()
            && weights.attn_q.m == q_dim
            && weights.attn_k.m == kv_dim
            && weights.attn_v.m == kv_dim;
        if lora.is_none()
            && valid_dims
...
pub(crate) fn forward_ffn_block(
    ...
    #[cfg(target_arch = "aarch64")]
    let fused_swiglu_done = activation == FfnActivation::Swiglu
        && lora.is_none()
    ...
    #[cfg(target_arch = "aarch64")]
    {
        ...
        } else if weights.ffn_gate.dtype == DType::Q4_0
...
mod decode_attn_tests {
    ...
    fn test_weight_ref_slice_rows_f32_and_quantized() {
        ...
        let s_q4k = w_q4k.slice_rows(1, 2).unwrap();
        assert_eq!(s_q4k.start, 5000 + 144);
        assert_eq!(s_q4k.size, 2 * 144);
        assert_eq!(s_q4k.m, 2);
        assert_eq!(s_q4k.k, 256);
        // Q4KM unaligned k (< 256)
        let w_q4k_bad = WeightRef::new(5000, 4 * 72, DType::Q4KM, 4, 128);
        assert!(w_q4k_bad.slice_rows(0, 2).is_err());

        // Q5KM test: k = 256 -> 1 superblock of 256 -> 176 bytes/row
        let w_q5k = WeightRef::new(7500, 4 * 176, DType::Q5KM, 4, 256);
        let s_q5k = w_q5k.slice_rows(2, 2).unwrap();
        assert_eq!(s_q5k.start, 7500 + 2 * 176);
        assert_eq!(s_q5k.size, 2 * 176);

        // Q6K test: k = 256 -> 1 superblock of 256 -> 210 bytes/row
        let w_q6k = WeightRef::new(9000, 4 * 210, DType::Q6K, 4, 256);
        let s_q6k = w_q6k.slice_rows(2, 2).unwrap();
        assert_eq!(s_q6k.start, 9000 + 2 * 210);
        assert_eq!(s_q6k.size, 2 * 210);

        // Q4_0 with k = 32 gives 1 block = 18 bytes/row (not a multiple of 4)
        let w_q4_odd = WeightRef::new(1000, 4 * 18, DType::Q4_0, 4, 32);
        let s_q4_odd = w_q4_odd.slice_rows(1, 2).unwrap();
        assert_eq!(s_q4_odd.start, 1000 + 18);
        assert_eq!(s_q4_odd.size, 36);
        assert_eq!(s_q4_odd.start % 4, 2);
