...
impl BailingMoe3Model {
    ...
    pub fn from_gguf_with_id(
        ...
        let rms_norm_eps = gguf
            ...
            rms_norm_eps > 0.0,
            "layer_norm_rms_epsilon must be positive"
        );
        ...
        let kda_lower_bound = gguf
            ...
            kda_lower_bound < 0.0,
            "kda.gate_lower_bound must be negative"
        );
        ...
        let rope_theta = gguf
            ...
            "key_length_mla must be >= rope.dimension_count"
        );
        ...
        for (il, &kv_heads) in head_count_kv.iter().enumerate().take(n_layers) {
            ...
            let kind = if kv_heads == 0 {
                ...
                let kv_a_norm = gguf
                    ...
                    "layer {il} attn_kv_a_norm len mismatch: expected {kv_lora_rank}"
                );
            ...
            let ffn = if il < leading_dense_block_count {
                    ...
                    gguf.get_tensor(&format!("blk.{il}.exp_probs_b.bias"))
                {
                    ...
                    "layer {il} exp_probs_b len mismatch: expected {n_expert}"
                );
                ...
                for e in 0..n_expert {
                    gate_exps.push(transformer::resolve_expert_weight(
                        &gguf,
                        &format!("blk.{il}.ffn_gate_exps.weight"),
                        e,
                    )?);
                    up_exps.push(transformer::resolve_expert_weight(
                        &gguf,
                        &format!("blk.{il}.ffn_up_exps.weight"),
                        e,
                    )?);
                    down_exps.push(transformer::resolve_expert_weight(
                        &gguf,
    ...
    fn forward(&self, tokens: &[u32], pos: usize, state: &mut InferenceState) -> Vec<f32> {
        ...
        if tokens.len() > 1 {
            tracing::warn!(
                ...
                "forward called with multiple tokens; prompt sequences should use forward_prefill"
            );
        }
        debug_assert_eq!(tokens.len(), 1, "forward() expects exactly 1 token");
