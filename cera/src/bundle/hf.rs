...
/// Helper to resolve known companion DSpark draft GGUF URLs published on Hugging Face.
///
...
pub fn resolve_hf_manifest(
    ...
    if is_audio_pipeline {
        ...
        if let Some(dec) = sidecar_fallbacks
            .iter()
            .find_map(|&target| {
                contents
                    .audio_decoders
            ...
            .or_else(|| contents.audio_decoders.first())
        {
            audio_decoder = Some(spec.file_download_url(&dec.rfilename));
        } else if let Some(voc) = sidecar_fallbacks
            .iter()
            .find_map(|&target| {
                contents
                    .vocoder_ggufs
            ...
            .or_else(|| contents.vocoder_ggufs.first())
        {
            audio_decoder = Some(spec.file_download_url(&voc.rfilename));
        }

        if let Some(tok_gguf) = sidecar_fallbacks
            .iter()
            .find_map(|&target| {
                contents
                    .tokenizer_ggufs
            ...
            .or_else(|| contents.tokenizer_ggufs.first())
        {
        ...
        if let Some(tok) = contents.tokenizer_ggufs.first() {
            extras.insert(
                "tokenizer".to_string(),
                spec.file_download_url(&tok.rfilename),
            );
        }
    }

    // Draft sidecar matching (co-located or standalone companion repo on HF)
    if !is_standalone_draft {
        if !contents.draft_ggufs.is_empty() {
        ...
        } else if let Some(companion_url) =
            known_companion_dspark_url(&spec.repo, &primary_entry.quant)
        {
            draft_model = Some(companion_url);
        }
    }
