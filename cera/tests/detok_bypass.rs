...
fn detok_bypass_backbone() {
    ...
    let tw = cera::model::audio_decoder::DetokenizerWeights::from_gguf(&gguf).unwrap();
    ...
    cera::backend::cpu::rmsnorm(&mut normed, &tw.output_norm, tw.config.rms_norm_eps);
    ...
    let pcm =
        cera::model::audio_decoder::istft_to_pcm(&spectrum, tw.config.n_fft, tw.config.hop_length);
    ...
    let dw = cera::model::audio_decoder::AudioDecoderWeights::from_gguf(&gguf).unwrap();
    let mut ts = cera::model::audio_decoder::DetokenizerState::new(&tw.config);
    let full_spectrum =
        cera::model::audio_decoder::detokenize_to_spectrum(&tw, &dw, &mut ts, &codes);
    let full_pcm = cera::model::audio_decoder::istft_to_pcm(
        &full_spectrum,
        tw.config.n_fft,
