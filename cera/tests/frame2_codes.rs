...
fn frame2_codes_from_ref_embedding() {
    ...
    let cera_f2 = load_emb("/tmp/cera_frame2_emb.bin");
    ...
    {
        let mut df = cera::model::audio_decoder::DepthformerState::new(&dw.depthformer_config);
        let c0 = cera::model::audio_decoder::sample_audio_frame(&dw, &mut df, &ref_f0, 0.0, 1);
        let c1 = cera::model::audio_decoder::sample_audio_frame(&dw, &mut df, &ref_f1, 0.0, 1);
        let c2 = cera::model::audio_decoder::sample_audio_frame(&dw, &mut df, &ref_f2, 0.0, 1);
        eprintln!("All ref embs → c0={c0:?} c1={c1:?} c2={c2:?}");
    }

    // Test 2: ref for frames 0-1, then CERA embedding for frame 2
    {
        let mut df = cera::model::audio_decoder::DepthformerState::new(&dw.depthformer_config);
        let c0 = cera::model::audio_decoder::sample_audio_frame(&dw, &mut df, &ref_f0, 0.0, 1);
        let c1 = cera::model::audio_decoder::sample_audio_frame(&dw, &mut df, &ref_f1, 0.0, 1);
        let c2 = cera::model::audio_decoder::sample_audio_frame(&dw, &mut df, &cera_f2, 0.0, 1);
        eprintln!("Ref f0+f1, cera f2 → c0={c0:?} c1={c1:?} c2={c2:?}");
    }

    // Test 3: all CERA embeddings (to see what cera actually does)
    // We only have cera's frame 1 and 2, not frame 0. Use ref's frame 0.
    {
        let cera_f1 = load_emb("/tmp/cera_frame1_emb.bin");
        let mut df = cera::model::audio_decoder::DepthformerState::new(&dw.depthformer_config);
        let c0 = cera::model::audio_decoder::sample_audio_frame(&dw, &mut df, &ref_f0, 0.0, 1);
        let c1 = cera::model::audio_decoder::sample_audio_frame(&dw, &mut df, &cera_f1, 0.0, 1);
        let c2 = cera::model::audio_decoder::sample_audio_frame(&dw, &mut df, &cera_f2, 0.0, 1);
        eprintln!("Ref f0, cera f1+f2 → c0={c0:?} c1={c1:?} c2={c2:?}");
    }
}
