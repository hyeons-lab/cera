//! Live smoke test for `cera::bundle::list_leap_bundles` against the
...
//! upstream API.
//!
//! Gating: `#[ignore]` + `CERA_TEST_DOWNLOAD=1` env var, matching
...
#[test]
#[ignore = "hits the real HF API; set CERA_TEST_DOWNLOAD=1 and pass --ignored"]
fn list_bundles_against_live_hf() {
    if std::env::var("CERA_TEST_DOWNLOAD").is_err() {
        eprintln!("skipping: CERA_TEST_DOWNLOAD not set");
        return;
    }
    let entries = cera::bundle::list_leap_bundles().expect("list_leap_bundles failed");
    assert!(
        !entries.is_empty(),
