...
//! or `cera-parity` (where every leg runs the same cera build, so
...
//!
//! ```sh
//! CERA_TEST_DOWNLOAD=1 cargo test -p cera \
...
//! never compiles it; `#[ignore]` + `CERA_TEST_DOWNLOAD=1` so even an
