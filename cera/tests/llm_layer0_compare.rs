...
fn llm_layer0_conv_standalone() {
    ...
    dump("/tmp/cera_llm_l0_embedding.bin", &hidden_orig);
    ...
    stat("normed", &normed);
    dump("/tmp/cera_llm_l0_normed.bin", &normed);

    // 2. in_proj → 3*hidden
    let mut proj = vec![0.0f32; 3 * hs];
    model.conv_in_proj_gemv(0, &normed, &mut proj);
    stat("in_proj", &proj);
    dump("/tmp/cera_llm_l0_in_proj.bin", &proj);
    ...
    dump("/tmp/cera_llm_l0_after_conv_residual.bin", &hidden);
    ...
    stat("FINAL", &output);
    dump("/tmp/cera_llm_l0_output.bin", &output);
