...
//! via `CERA_FLASH=1`), `attention_gqa.metal` (`CERA_ATTN=gqa`), and
//! `attention_splitk.metal` (`CERA_ATTN=splitk`) — must all produce
...
//!
//! Run with:
//!   cargo test -p cera --release --features metal --test attention_metal_parity -- --ignored --nocapture
...
/// generated-token slice. Constructs a fresh model so the `CERA_FLASH` env
...
fn test_classic_vs_flash_attention_parity() {
    ...
    let classic = with_env("CERA_FLASH", None, || {
        generate_greedy(&path, PROMPT, N_TOKENS)
    });
    let flash = with_env("CERA_FLASH", Some("1"), || {
        generate_greedy(&path, PROMPT, N_TOKENS)
    });
    assert_parity(&classic, &flash, "flash");
}

/// GQA attention (`CERA_ATTN=gqa`) must match classic. Same f16/f32 bug
...
fn test_classic_vs_gqa_attention_parity() {
    ...
    let classic = with_env("CERA_ATTN", None, || {
        generate_greedy(&path, PROMPT, N_TOKENS)
    });
    let gqa = with_env("CERA_ATTN", Some("gqa"), || {
        generate_greedy(&path, PROMPT, N_TOKENS)
    });
    assert_parity(&classic, &gqa, "gqa");
}

/// Split-K attention (`CERA_ATTN=splitk`) must match classic. Same
...
fn test_classic_vs_splitk_attention_parity() {
    ...
    let splitk = with_env("CERA_ATTN", Some("splitk"), || {
        generate_greedy(&path, PROMPT, N_TOKENS)
    });
