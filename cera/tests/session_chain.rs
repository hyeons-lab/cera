...
//!
//! Run with:
//!   CERA_MODEL=$HOME/.leap/models/LFM2-VL-450M-Q4_0/LFM2-VL-450M-Q4_0.gguf \
//!   cargo test -p cera --test session_chain -- --ignored --nocapture
//!
...
fn find_model() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("CERA_MODEL") {
...
/// intermediate `append_tokens` returns `CeraError::EmptyInput`. Adding an
...
fn greedy_chain_requires_append_tokens() {
    ...
    let err = session
        ...
        .unwrap_err();
    assert!(
        matches!(err, cera::CeraError::EmptyInput),
...
fn stochastic_split_matches_single_call_under_seed() {
    ...
    let baseline = {
        let gguf = cera::gguf::GgufFile::open(&model_path).unwrap();
        let model: Arc<dyn Model> = Arc::from(cera::model::load_model(gguf, None, 4096).unwrap());
...
fn forward_prefill_from_embeddings_matches_per_frame_loop() {
    ...
    let model_a = cera::model::load_model(gguf_a, None, 4096).unwrap();
    let model_b = cera::model::load_model(gguf_b, None, 4096).unwrap();
...
fn append_audio_text_only_returns_unsupported_modality() {
    ...
    let gguf = cera::gguf::GgufFile::open(&model_path).unwrap();
    let tokenizer = cera::tokenizer::BpeTokenizer::from_gguf(&gguf).unwrap();
    let model = cera::model::load_model(gguf, None, 1024).unwrap();
...
fn append_audio_wrong_sample_rate_returns_backend_error() {
    use cera::model::audio_encoder::AudioEncoderWeights;
...
fn append_audio_end_to_end() {
    use cera::model::audio_encoder::{AudioEncoderWeights, SAMPLE_RATE};
    ...
    let primary_gguf = cera::gguf::GgufFile::open(&primary).unwrap();
    let tokenizer = cera::tokenizer::BpeTokenizer::from_gguf(&primary_gguf).unwrap();
    let model = cera::model::load_model(primary_gguf, None, 2048).unwrap();
    let model: Arc<dyn Model> = Arc::from(model);
    let tokenizer = Arc::new(tokenizer);
    let mut session = Session::new(
        model,
...
fn reset_preserves_attached_audio_encoder() {
    ...
    match session.append_audio(&pcm, 16_000) {
        Err(cera::CeraError::Backend(msg)) => {
            assert!(
...
/// End-to-end engine auto-attach: build a `CeraEngine` from the
...
fn engine_auto_attaches_audio_encoder_from_bundle() {
    use cera::model::audio_encoder::SAMPLE_RATE;
    ...
    let engine = CeraEngine::from_path(&bundle_dir, EngineConfig::default()).unwrap();
    assert!(
...
fn engine_does_not_load_encoder_for_text_bundle() {
    ...
    let engine = CeraEngine::from_path(&model_path, EngineConfig::default()).unwrap();
    assert!(
...
//
// External TTS (rather than cera's own) so a future LLM
// regression doesn't shift both directions of a self-loop in
// lockstep — see `cera/tests/fixtures/audio/README.md` for the
// independent-oracle rationale.
// ---------------------------------------------------------------------------

/// Read a 16 kHz mono PCM16 WAV file into f32 samples in [-1, 1].
/// Mirrors `cera-cli/src/main.rs::read_wav_pcm16_mono` — duplicated
/// here so the test doesn't depend on the CLI binary. Errors out
/// on any format other than the canonical cera audio-input shape.
///
...
fn asr_real_audio_matches_input_phrase() {
    ...
    let primary_gguf = cera::gguf::GgufFile::open(&primary).unwrap();
    let tokenizer = cera::tokenizer::BpeTokenizer::from_gguf(&primary_gguf).unwrap();
    let model = cera::model::load_model(primary_gguf, None, 4096).unwrap();
    let model: Arc<dyn Model> = Arc::from(model);
    ...
    // Chat-template flow (mirrors `cera-cli/src/main.rs`'s
    ...
    // Match `cera-cli`'s `split_at_marker` shape: count occurrences
...
fn ignore_eos_forces_exact_token_count() {
    ...
    assert_eq!(
        summary.tokens_generated, 1,
        "greedy continuation of the probe prompt should emit one token"
    );
    let first_token = sink.0[0];
    // prompt_eval_tokens pairs with prompt_eval_ms and equals the tokens
    // prefilled since the last generate — deterministic, unlike the wall-clock
    // ms which can truncate to 0 for a sub-millisecond prefill.
    assert_eq!(summary.prompt_eval_tokens, prompt_toks.len() as u32);
    // generate() consumed the accumulator: appending N more tokens and
    // generating again reports exactly N, not the original prompt too.
    let suffix = &prompt_toks[..prompt_toks.len().min(2)];
    session.append_tokens(suffix).unwrap();
    let mut sink_chain = CollectSink(Vec::new());
    let summary2 = session.generate(&greedy_opts(1), &mut sink_chain).unwrap();
    assert_eq!(summary2.prompt_eval_tokens, suffix.len() as u32);
...
/// `ignore_eos` must NOT apply while a grammar is actively constraining: the
/// grammar's EOS-on-completion stop still fires, so constrained output stays
/// well-formed (no stray EOS token, no `GrammarDeadEnd`). Without the
/// `honor_stop = armed || !ignore_eos` guard, decode would skip the stop, emit
/// EOS into the stream, feed it to the completed `GrammarState`, and finish
/// `GrammarDeadEnd`.
#[test]
#[ignore]
fn ignore_eos_does_not_override_active_grammar() {
    use std::sync::Arc;
    ...
    let tokenizer = cera::tokenizer::BpeTokenizer::from_gguf(&gguf).unwrap();
    let model = cera::model::load_model(gguf, None, 4096).unwrap();
    let prompt_toks = tokenizer.encode("Answer:");

    let mut session = make_session(model, tokenizer, SessionConfig::default());
    session.append_tokens(&prompt_toks).unwrap();

    // A single-word grammar: decode is forced to emit exactly "yes", after
    // which only EOS is permitted.
    let grammar = Arc::new(cera::grammar::Grammar::parse(r#"root ::= "yes""#).unwrap());
    let opts = GenerateOpts {
        max_tokens: 32,
        temperature: 0.0,
    ...
    // Stops cleanly on the grammar's EOS, well before max_tokens, and never
    // dead-ends or leaks EOS into the output.
    assert!(
        matches!(summary.finish_reason, FinishReason::Stop),
        "expected clean Stop, got {:?}",
        summary.finish_reason
    );
