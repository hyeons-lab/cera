...
//! encoder. Runs cera's vision encoder on a fixed input
...
//! would land in `cera-parity` once we capture the full vector.
//!
//! Gating: `#[ignore]` + `CERA_TEST_DOWNLOAD=1` to share the same
...
fn vl_clip_parity_smoke() {
    ...
    let pre = cera::model::vision_preprocessor::preprocess_image(&png, &encoder.config)
    ...
    eprintln!("cera : mean={mean:.4} std={std:.4} min={mn:.4} max={mx:.4}");
