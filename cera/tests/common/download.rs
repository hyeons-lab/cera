...
//!
//! Thin shim over `cera::bundle::BundleRepo` — the download logic used
...
//! helpers).
//!
//! Cache root: `$CERA_TEST_MODELS_DIR` if set, else
//! `target/tmp/cera-test-models` under the workspace root. A
...
//!
//! Used only by tests gated on `CERA_TEST_DOWNLOAD=1` — never runs in a
...
pub fn cache_dir() -> PathBuf {
    if let Ok(override_path) = std::env::var("CERA_TEST_MODELS_DIR") {
        let p = PathBuf::from(override_path);
        fs::create_dir_all(&p).expect("create CERA_TEST_MODELS_DIR");
        return p;
    }
    // Workspace-root-relative. `CARGO_MANIFEST_DIR` points at the crate
    // (`cera/`), so one dir up is the workspace root.
