...
#[test]
fn phi3_architecture_alias_parity() {
    let Some(path) = ensure_test_fixture() else {
        return;
    };
    let gguf = GgufFile::open(&path).expect("open test_phi3.gguf");

    // 1. Architecture alias with phi3.* metadata keys (fallback)
    let mut phi_alias_gguf = gguf.clone();
    phi_alias_gguf.metadata.insert(
        "general.architecture".to_string(),
        cera::gguf::GgufValue::String("phi".to_string()),
    );

    // 2. Architecture alias with native phi.* metadata keys
    let mut phi_native_gguf = gguf.clone();
    phi_native_gguf.metadata.insert(
        "general.architecture".to_string(),
        cera::gguf::GgufValue::String("phi".to_string()),
    );
    let phi3_keys: Vec<(String, cera::gguf::GgufValue)> = phi_native_gguf
        .metadata
        .iter()
        .filter_map(|(k, v)| {
            k.strip_prefix("phi3.")
                .map(|rest| (format!("phi.{rest}"), v.clone()))
        })
        .collect();
    for (k, v) in phi3_keys {
        phi_native_gguf.metadata.insert(k, v);
    }
    phi_native_gguf
        .metadata
        .retain(|k, _| !k.starts_with("phi3."));

    let model_phi3 = LlamaModel::from_gguf(gguf, 256).expect("load phi3 model");
    let model_phi = LlamaModel::from_gguf(phi_alias_gguf, 256).expect("load phi alias model");
    let model_phi_native =
        LlamaModel::from_gguf(phi_native_gguf, 256).expect("load phi native model");

    let prompt = [1u32, 10, 20, 30];
    let mut state_phi3 =
        InferenceState::from_config_with_compression(model_phi3.config(), &KvCompression::None)
            .unwrap();
    let mut state_phi =
        InferenceState::from_config_with_compression(model_phi.config(), &KvCompression::None)
            .unwrap();
    let mut state_phi_native = InferenceState::from_config_with_compression(
        model_phi_native.config(),
        &KvCompression::None,
    )
    .unwrap();

    let logits_phi3 = model_phi3.forward_prefill(&prompt, 0, &mut state_phi3);
    let logits_phi = model_phi.forward_prefill(&prompt, 0, &mut state_phi);
    let logits_phi_native = model_phi_native.forward_prefill(&prompt, 0, &mut state_phi_native);

    assert_eq!(logits_phi3.len(), logits_phi.len());
    assert_eq!(logits_phi3.len(), logits_phi_native.len());

    let sim_alias = compute_cosine_similarity(&logits_phi3, &logits_phi);
    let sim_native = compute_cosine_similarity(&logits_phi3, &logits_phi_native);

    assert!(
        sim_alias > 0.99999,
        "phi alias logits diverge from phi3 (cosine sim = {sim_alias})"
    );
    assert!(
        sim_native > 0.99999,
        "phi native logits diverge from phi3 (cosine sim = {sim_native})"
    );
}

#[test]
fn phi3_sliding_window_attention_parity() {
    let Some(path) = ensure_test_fixture() else {
        return;
    };
    let gguf = GgufFile::open(&path).expect("open test_phi3.gguf");
    let mut swa_gguf = gguf.clone();
    swa_gguf.metadata.insert(
        "phi3.attention.sliding_window".to_string(),
        cera::gguf::GgufValue::U32(2),
    );

    let model_swa = LlamaModel::from_gguf(swa_gguf, 256).expect("load phi3 swa model");
    assert_eq!(model_swa.sliding_window(), Some(2));

    let prompt = [1u32, 5, 15, 25, 35];
    let mut state =
        InferenceState::from_config_with_compression(model_swa.config(), &KvCompression::None)
            .unwrap();

    let prefill_logits = model_swa.forward_prefill(&prompt, 0, &mut state);
    assert!(!prefill_logits.is_empty());
    assert_eq!(state.seq_len, prompt.len());

    // Single token decode continuation
    let next_token = 42u32;
    let decode_logits = model_swa.forward(&[next_token], state.seq_len, &mut state);
    assert!(!decode_logits.is_empty());
    assert_eq!(state.seq_len, prompt.len() + 1);
}
