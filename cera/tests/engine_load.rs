//! Integration coverage for `CeraEngine`'s loader dispatch.
//!
//! The tests here focus on **control flow** — manifest shape → the
//! right `CeraError` variant or the right load path — not on actually
...
use cera::{BackendPreference, CeraEngine, CeraError, EngineConfig, ModelFiles};
...
/// `CeraEngine` can't derive `Debug` (holds `Box<dyn Model>`). Shim
/// around `.expect_err` that does the same job: pattern-match the
/// `Err` and panic with `msg` otherwise.
#[track_caller]
fn expect_err(r: Result<CeraEngine, CeraError>, msg: &str) -> CeraError {
    match r {
        Ok(_) => panic!("{msg}"),
        Err(e) => e,
    }
...
fn from_path_rejects_remote_model_url() {
    ...
    let err = expect_err(
        CeraEngine::from_path(&manifest, cpu_cfg()),
...
fn from_path_vl_manifest_passes_inference_type_gate() {
    ...
    assert!(
        !matches!(err, CeraError::UnsupportedInferenceType(_)),
...
fn from_path_unknown_inference_type_errors_with_raw_string() {
    ...
    match err {
        CeraError::UnsupportedInferenceType(s) => {
...
fn from_path_directory_with_zero_manifests_errors() {
    let dir = tempfile::tempdir().unwrap();
    let err = expect_err(
        CeraEngine::from_path(dir.path(), cpu_cfg()),
...
fn from_path_rejects_arbitrary_extension() {
    ...
    let err = expect_err(
        CeraEngine::from_path(&odd, cpu_cfg()),
...
fn find_model() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("CERA_TEST_MODEL") {
...
#[test]
#[ignore = "requires CERA_TEST_MODEL pointing at a local .gguf"]
fn from_path_on_gguf_loads_text_engine() {
    let Some(model_path) = find_model() else {
        return;
    };
    let engine = CeraEngine::from_path(&model_path, cpu_cfg()).expect("load from .gguf");
    assert_eq!(
...
fn from_files_equivalent_to_from_path_for_text_gguf() {
    ...
    let via_path = CeraEngine::from_path(&model_path, cpu_cfg()).expect("from_path load");
    let via_files =
        CeraEngine::from_files(ModelFiles::text(&model_path), cpu_cfg()).expect("from_files load");
