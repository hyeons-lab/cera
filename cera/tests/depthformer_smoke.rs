...
fn depthformer_forward_smoke() {
    ...
    let mut state = cera::model::audio_decoder::DepthformerState::new(&weights.depthformer_config);
    ...
    for step in 0..3 {
        ...
        let out = cera::model::audio_decoder::depthformer_forward(&weights, &mut state, &input);
    ...
    state.reset();
    let out2 = cera::model::audio_decoder::depthformer_forward(&weights, &mut state, &input);
