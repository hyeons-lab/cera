...
//! Run with:
//! ```text
//! CERA_QUALITY_GATE=1 cargo test -p cera --release --test quality_gate -- --ignored --nocapture
//! ```
//!
...
#[test]
#[ignore] // run with --ignored + CERA_QUALITY_GATE=1
fn test_kv_compression_quality_gate() {
    if std::env::var("CERA_QUALITY_GATE").as_deref() != Ok("1") {
        eprintln!("skipping: CERA_QUALITY_GATE=1 not set");
        return;
    }
