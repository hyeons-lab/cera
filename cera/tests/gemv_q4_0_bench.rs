...
//!
//! Run with:
//!   cargo test -p cera --features "gpu metal" --release \
//!     --test gemv_q4_0_bench -- --ignored --nocapture
//!
//! Decides whether porting the cera forward pass to native Metal is justified.

#![cfg(all(feature = "gpu", feature = "metal", target_os = "macos"))]

use cera::backend::{metal::MetalContext, wgpu::GpuContext};
...
fn bench_metal(
    ...
    // Switch via env: CERA_Q4_FAST=1 to bench the fast variant.
    let mode = std::env::var("CERA_Q4_FAST").unwrap_or_default();
    if mode == "splitk" {
    ...
    let (pipeline, groups, threads) = if mode == "slim" {
        (
            ctx.create_pipeline(
                cera::backend::metal::shaders::GEMV_Q4_0_FAST,
                "gemv_q4_0_fast_slim",
            )
            .expect("MSL compile"),
        ...
        (
            ctx.create_pipeline(cera::backend::metal::shaders::GEMV_Q4_0, "gemv_q4_0")
                .expect("MSL compile"),
            shape.m.div_ceil(2) as u64,
            32u64,
        )
