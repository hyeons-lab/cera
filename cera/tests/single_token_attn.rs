...
fn single_token_attention() {
    ...
    let gguf = cera::gguf::GgufFile::open_arc(&vocoder_path).unwrap();
    ...
    cera::backend::cpu::rmsnorm(&mut cur, &lw.operator_norm, cfg.rms_norm_eps);
    ...
    let mut df_state = cera::model::audio_decoder::DepthformerState::new(cfg);
    let hidden = cera::model::audio_decoder::depthformer_forward(&dw, &mut df_state, &input);
