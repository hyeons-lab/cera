...
#[test]
fn bailingmoe3_forward_rejects_divergent_pos() {
    let Some(path) = ensure_test_fixture() else {
        return;
    };
    ...
    // state.seq_len is 0, but pos is 5
    let logits = model.forward(&[1], 5, &mut state);
    assert!(
        logits.is_empty(),
        "forward must return empty logits when pos diverges from state.seq_len"
    );
    assert_eq!(
        state.seq_len, 0,
        "state.seq_len must not advance when forward rejected divergent pos"
    );
}

#[test]
fn bailingmoe3_multi_token_forward_delegates_to_prefill() {
    let Some(path) = ensure_test_fixture() else {
        return;
    };
    ...
    let tokens = [1u32, 65, 66];
    let logits_forward = model.forward(&tokens, 0, &mut state_forward);
    ...
    assert!(
        sim > 0.99999,
        "multi-token forward must delegate to prefill: sim = {sim}"
    );
    assert_eq!(state_forward.seq_len, tokens.len());
    assert_eq!(state_prefill.seq_len, tokens.len());
}

#[test]
fn bailingmoe3_context_size_bounding() {
    let Some(path) = ensure_test_fixture() else {
        return;
    };

    let gguf = GgufFile::open(&path).expect("open test_bailingmoe3.gguf");
    // Model fixture context_length is 256; requested context_size is 64
    let model = BailingMoe3Model::from_gguf(gguf, 64).expect("load model");
    assert_eq!(
        model.config().max_seq_len,
        64,
        "caller requested context size must bound model max_seq_len"
    );
}

#[test]
fn bailingmoe3_select_experts_nan_isolation() {
    let mut selected = Vec::new();
    // Expert 0 has NaN prob, experts 1, 2, 3 have real scores
    let probs = [f32::NAN, 0.8, 0.6, 0.2];
    let biases = [0.0; 4];
    cera::model::bailingmoe3::select_bailingmoe_experts(&probs, &biases, 2, 1.0, &mut selected);

    assert_eq!(selected.len(), 2);
    assert_eq!(
        selected[0].0, 1,
        "highest valid expert should be selected first"
    );
    assert_eq!(
        selected[1].0, 2,
        "second valid expert should be selected second"
    );
    assert!(
        selected
            .iter()
            .all(|&(idx, w)| idx != 0 && w > 0.0 && w.is_finite())
    );
}

#[test]
fn bailingmoe3_rejects_non_positive_or_non_finite_rope_theta() {
    let Some(path) = ensure_test_fixture() else {
        return;
    };
    ...
    let mut gguf = GgufFile::open(&path).expect("open test_bailingmoe3.gguf");
    gguf.metadata.insert(
        "bailingmoe3.rope.freq_base".to_string(),
        cera::gguf::GgufValue::F32(0.0),
    );
    assert!(
        BailingMoe3Model::from_gguf(gguf, 256).is_err(),
        "should reject zero rope_theta"
    );

    let mut gguf = GgufFile::open(&path).expect("open test_bailingmoe3.gguf");
    gguf.metadata.insert(
        "bailingmoe3.rope.freq_base".to_string(),
        cera::gguf::GgufValue::F32(f32::NAN),
    );
    assert!(
        BailingMoe3Model::from_gguf(gguf, 256).is_err(),
        "should reject NaN rope_theta"
    );
}

#[test]
fn bailingmoe3_rejects_non_positive_or_non_finite_expert_weights_scale() {
    let Some(path) = ensure_test_fixture() else {
        return;
    };
    ...
    let mut gguf = GgufFile::open(&path).expect("open test_bailingmoe3.gguf");
    gguf.metadata.insert(
        "bailingmoe3.expert_weights_scale".to_string(),
        cera::gguf::GgufValue::F32(-1.0),
    );
    assert!(
        BailingMoe3Model::from_gguf(gguf, 256).is_err(),
        "should reject negative expert_weights_scale"
    );

    let mut gguf = GgufFile::open(&path).expect("open test_bailingmoe3.gguf");
    gguf.metadata.insert(
        "bailingmoe3.expert_weights_scale".to_string(),
        cera::gguf::GgufValue::F32(f32::INFINITY),
    );
    assert!(
        BailingMoe3Model::from_gguf(gguf, 256).is_err(),
        "should reject infinite expert_weights_scale"
    );
}

#[test]
fn bailingmoe3_rejects_qk_head_dim_equal_to_qk_rope_head_dim() {
    let Some(path) = ensure_test_fixture() else {
        return;
    };

    let mut gguf = GgufFile::open(&path).expect("open test_bailingmoe3.gguf");
    // In fixture: rope.dimension_count is 4. Setting key_length_mla to 4 causes qk_head_dim == qk_rope_head_dim.
    gguf.metadata.insert(
        "bailingmoe3.attention.key_length_mla".to_string(),
        cera::gguf::GgufValue::U32(4),
    );
    assert!(
        BailingMoe3Model::from_gguf(gguf, 256).is_err(),
        "should reject qk_head_dim == qk_rope_head_dim"
    );
}

#[test]
fn bailingmoe3_rejects_non_positive_or_non_finite_rms_norm_eps() {
    let Some(path) = ensure_test_fixture() else {
        return;
    };
    ...
    let mut gguf = GgufFile::open(&path).expect("open test_bailingmoe3.gguf");
    gguf.metadata.insert(
        "bailingmoe3.attention.layer_norm_rms_epsilon".to_string(),
        cera::gguf::GgufValue::F32(0.0),
    );
    assert!(
        BailingMoe3Model::from_gguf(gguf, 256).is_err(),
        "should reject zero rms_norm_eps"
    );
    ...
    assert!(
        BailingMoe3Model::from_gguf(gguf, 256).is_err(),
        "should reject NaN rms_norm_eps"
    );
}
