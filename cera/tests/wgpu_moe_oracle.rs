//! wgpu mixture-of-experts oracle: the routed FFN pinned against the CPU one.
//!
//! The wgpu twin of `tests/metal_moe_oracle.rs`, and it exists for the same
//! reason: `tests/slang_multitarget_parity.rs` pins the three MoE kernels
//! (`moe_route`, `moe_gemv_q4_0`, `moe_combine`) against CPU references on
//! synthetic data, including exact expert-id agreement, but nothing there says
//! the *model* wires them together correctly on a real `lfm2moe` GGUF, with the
//! right stacked-tensor strides, the right per-layer router and bias, and the
//! dense leading blocks still taking the dense path.
//!
//! Both backends run the same Slang source through different emitters, so the
//! kernels are shared and the wiring is not: the host code here is a separate
//! implementation with its own buffer bindings, its own dispatch grids, and its
//! own prefill convention. That is the part this file covers.
//!
...
//! difference between the CPU and GPU hidden states feeding the router. When the
//! boundary gap is smaller than that difference the two backends select a
//! different fourth expert, and from there they are computing genuinely
//! different, equally valid functions. No tolerance on the output can paper that
//! over, so the test states it. See `metal_moe_oracle`'s module docs for the
//! measurement behind the bounds; llama.cpp shows the same effect on this model.
//!
//! ## The assertion that would catch a real bug
//!
//! The first token is the load-bearing one. With a single position there is no
//! attention history, so the CPU and GPU hidden states entering the first routed
//! layer agree to ~1e-4, no boundary flips, and the routed FFN has to reproduce
//! the CPU result to dense-model accuracy. A wrong expert stride, a transposed
//! activation index, or a router read from the wrong layer all show up there
//! immediately.
//!
//! Gated on an `lfm2moe` GGUF via `CERA_LFM2MOE_MODEL`. Run:
//!   CERA_LFM2MOE_MODEL=... cargo test -p cera --release --features gpu \
//!     --test wgpu_moe_oracle -- --ignored --nocapture
...
use cera::model::{BlockType, Model, load_model, load_model_gpu};
...
///
/// Hard-coded rather than run through the tokenizer so the fixture cannot change
/// underneath the bounds below, and identical to `metal_moe_oracle`'s so the two
/// backends' numbers are comparable.
...
///
/// Deliberately not `hidden_states`: that is a token-at-a-time loop over the
/// decode encoders, so it exercises the routed FFN's decode arm only. The
/// batched-prefill arm is a different call with a different output convention
/// (`accumulate = false`, its residual folded in by the *next* layer), and it is
/// the arm normal generation runs the prompt through.
...
/// Load the CPU and wgpu models for one test.
///
/// `None` means only that no model was configured. A load that *fails* panics
/// instead: `model_path` has already proved the file exists and parses, so every
/// remaining failure is a regression in the thing under test (the Q4_0-only
/// expert check, the shape validation, a missing tensor), and returning `None`
/// for those would print "SKIP: set CERA_LFM2MOE_MODEL" and pass the suite green
/// on a broken loader.
fn both() -> Option<Pair> {
    let path = model_path()?;
    ...
    let gpu = load_model_gpu(open(), Some(&path), 4096).expect("lfm2moe loads on the wgpu backend");
    ...
    Some(Pair { cpu, gpu, hs })
}

/// The first token, where routing is not yet ambiguous, must match the CPU to
/// the same accuracy a dense model does.
///
/// This is the assertion with teeth. Every structural mistake the routed path can
/// make (an expert stride that lands on a neighbour's weights, the gate/up
/// activation index read per entry instead of per token, the router or bias taken
/// from the wrong layer) changes this token's output far beyond the bound below,
/// because it changes it for *every* layer rather than for the one marginal
/// expert a boundary flip moves.
#[test]
#[ignore = "needs an lfm2moe GGUF via CERA_LFM2MOE_MODEL"]
fn first_token_matches_cpu_to_dense_accuracy() {
    let Some(Pair { cpu, gpu, hs }) = both() else {
        eprintln!("[wgpu-moe-oracle] SKIP: set CERA_LFM2MOE_MODEL");
        return;
    };
    let want = hidden(cpu.as_ref(), &TOKENS[..1]);
    let got = hidden(gpu.as_ref(), &TOKENS[..1]);
    ...
    eprintln!("[wgpu-moe-oracle] first token cosine {cos:.6}");
}

/// Across a full prompt the two backends stay close, but not dense-close, and the
/// gap is routing divergence rather than arithmetic error.
///
/// The floor is deliberately loose: too low and a real regression hides under it,
/// but asserting a dense-model bound here would fail on correct code the first
/// time a boundary flip landed.
#[test]
#[ignore = "needs an lfm2moe GGUF via CERA_LFM2MOE_MODEL"]
fn full_prompt_stays_within_the_routing_divergence_bound() {
    ...
    let got = hidden(gpu.as_ref(), TOKENS);
    ...
    eprintln!(
        "[wgpu-moe-oracle] worst per-token cosine {:.6} at token {}",
        worst.0, worst.1
    );
    assert!(
        worst.0 > 0.9,
...
/// The batched-prefill routed FFN agrees with the CPU, and with wgpu's own decode
/// path.
///
/// The other two tests reach the routed FFN only through `hidden_states`, which
/// loops token-at-a-time over the decode encoders. The prefill arm is a separate
/// call with a convention decode does not share: it *overwrites* its output
/// scratch rather than accumulating into the residual, leaving the next layer's
/// fused `add_rmsnorm_batch` to fold it in. That is exactly the kind of thing
/// that is correct on one path and wrong on the other, and normal generation runs
/// every prompt through this one.
///
...
/// apply. Against wgpu's own decode of the same prompt the bound is tight: one
/// backend and one set of weights, so a dropped or double-counted residual shows
/// up as a gross mismatch rather than a near miss.
///
/// The tight bound rests on the two paths *expecting* the same routing, not on a
/// guarantee of it. Batched GEMM and flash attention are not bit-identical to the
/// decode GEMV and split attention, so the router input differs by ~1e-6, which
/// is orders below the ~1e-3 top-k boundary gaps that make the CPU comparison
/// loose. If this ever flakes, the cause to rule out first is a genuine boundary
/// flip, not the wiring.
#[test]
#[ignore = "needs an lfm2moe GGUF via CERA_LFM2MOE_MODEL"]
fn batched_prefill_agrees_with_decode_and_cpu() {
    let Some(Pair { cpu, gpu, .. }) = both() else {
        eprintln!("[wgpu-moe-oracle] SKIP: set CERA_LFM2MOE_MODEL");
        return;
    };

    let cpu_logits = prefill_logits(cpu.as_ref(), TOKENS);
    let gpu_logits = prefill_logits(gpu.as_ref(), TOKENS);
    assert_eq!(cpu_logits.len(), gpu_logits.len());

    // wgpu's own decode of the same prompt: feed the tokens one at a time, so
    // the last call's logits predict the same next token the prefill does.
    //
    // On a *fresh* model, not the one just prefilled. Like Metal, this backend
    // keeps its KV cache and the shortconv rolling buffers on the model rather
    // than in `InferenceState`, so replaying from position 0 on the prefilled
    // instance would decode against a dirty cache and read as a kernel bug.
    let decode_logits = {
        let path = model_path().expect("both() already resolved it");
        let fresh = load_model_gpu(
    ...
    let vs_decode = cosine(&gpu_logits, &decode_logits);
    let vs_cpu = cosine(&gpu_logits, &cpu_logits);
    eprintln!(
        "[wgpu-moe-oracle] prefill logits cosine {vs_decode:.6} vs wgpu decode, \
...
/// wgpu refuses a routed-FFN adapter instead of applying half of it.
///
/// Driven through `Session::attach_lora_adapters`, not through the
/// `supports_moe_lora` flag it reads: asserting the flag would pass even with the
/// rejection in `session.rs` deleted, which is the failure this test exists to
/// catch. The predicate behind it, including that a *router* delta counts as a
/// routed-FFN delta, is pinned separately by
...
#[test]
#[ignore = "needs an lfm2moe GGUF via CERA_LFM2MOE_MODEL"]
fn wgpu_session_rejects_a_router_lora() {
    ...
    let model: Arc<dyn Model> =
        Arc::from(load_model_gpu(gguf, Some(&path), 512).expect("lfm2moe loads on wgpu"));
    let (hidden_size, n_expert, routed_layer) = {
        ...
        // "mixture-of-experts", and the test would pass without ever reaching the
        // gate it exists to cover.
        let routed = moe
            .is_moe_layer
            .iter()
            .position(|&b| b)
    ...
    let err = session
        ...
        .expect_err("wgpu has no routed-FFN hooks, so this adapter must be refused");
    ...
    eprintln!("[wgpu-moe-oracle] router adapter refused: {err}");
}

/// An adapter set straight onto `InferenceState` is dropped whole, not applied
/// in half.
///
/// `wgpu_session_rejects_a_router_lora` covers the gate a normal caller meets.
...
fn a_routed_adapter_on_the_state_is_dropped_whole() {
    ...
    eprintln!("[wgpu-moe-oracle] routed adapter on the state was dropped whole");
}
