...
//!
//! What IS covered:
//! - Real LFM2-350M-Extract-Q4_0 loads via [`CeraEngine::from_bundle_id`].
...
//! - At least one `cera::kv_shift` tracing event fires.
...
/// Counts the `shift` events emitted on `cera::kv_shift` via a tracing
...
fn run_shift_through_real_model(backend: BackendPreference) {
    ...
    // `cera-parity` and `bundle_from_id.rs` exercise; the shared cache
    // under `cera-test-models/` means at most one HTTP fetch per run.
