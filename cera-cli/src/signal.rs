...
//! line-mode `cera chat --no-tui` path where Ctrl+C would otherwise kill
...
//! `CeraError::Cancelled` / `FinishReason::Cancelled`; the REPL pops the
...
mod tests {
    ...
    // Manual smoke covers it: spawning `cera chat --no-tui` and sending
