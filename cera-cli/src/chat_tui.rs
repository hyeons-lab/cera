//! Inline TUI for `cera chat` — Claude Code / Junie style.
//!
...
use cera::{FinishReason, GenerateOpts, ModalitySink, Session};
...
enum UiUpdate {
    ...
    /// local history copy after `CeraError::ContextOverflow`.
...
/// clone because `CeraEngine` already stores the tokenizer behind an
...
fn worker_loop(
    ...
    while let Ok(req) = rx_turn.recv() {
        ...
        //
        // On `CeraError::ContextOverflow` we drop the oldest
        ...
        let prefill_outcome: Result<(), String> = loop {
            ...
            let attempt: Result<(), cera::CeraError> = if any_images {
                ...
                let messages: Vec<cera::tokenizer::ChatMessageMultimodal> = history
                    .iter()
                ...
                match cera::tokenizer::apply_chat_template(&tokenizer, &history, true) {
                    Ok(formatted) => {
            ...
            match attempt {
                Ok(()) => break Ok(()),
                Err(cera::CeraError::ContextOverflow { max_seq_len, by }) => {
