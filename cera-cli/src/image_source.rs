//! Image-input loader shared by `cera run --image`, the line-REPL `/image`
...
/// waste that grows linearly with `cera run --image url1 --image url2 …`.
fn http_client() -> &'static reqwest::blocking::Client {
    ...
    CLIENT.get_or_init(|| {
        reqwest::blocking::Client::builder()
            ...
            .user_agent(concat!("cera-cli/", env!("CARGO_PKG_VERSION")))
            .build()
