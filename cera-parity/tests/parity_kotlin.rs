...
//! and the vendored Kotlin runner under `cera-parity/legs/kotlin/`,
...
//!   - `CERA_PARITY_KOTLIN_RUNNER=<path/to/cera-parity-kotlin-all.jar>` —
//!     points at the fat jar produced by
//!     `cd cera-parity/legs/kotlin && ./gradlew shadowJar`.
//!
//! Optional:
//!   - `CERA_PARITY_LIB_DIR=<path/to/dir/containing/libcera_ffi>` —
...
#[test]
#[ignore = "downloads ~210 MB + needs CERA_PARITY_KOTLIN_RUNNER set"]
fn rust_and_kotlin_jna_produce_identical_tokens() {
    ...
    let runner = match std::env::var("CERA_PARITY_KOTLIN_RUNNER") {
        Ok(p) => PathBuf::from(p),
        Err(_) => {
            eprintln!("skipping: CERA_PARITY_KOTLIN_RUNNER not set");
            return;
        }
    };
    assert!(
        runner.exists(),
        "CERA_PARITY_KOTLIN_RUNNER points at {} which does not exist",
        runner.display()
    );
    ...
    let (kotlin, kotlin_ms) =
        cera_parity::run_kotlin_jna(&args, &runner, &lib_dir).expect("kotlin-jna path");
    eprintln!("perf: rust={rust_ms:?} kotlin-jna={kotlin_ms:?}");

    if let Some(idx) = cera_parity::first_divergence(&rust, &kotlin) {
