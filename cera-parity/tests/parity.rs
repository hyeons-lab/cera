//! Real-model parity check.
//!
//! Downloads `LFM2-350M-Extract-GGUF/Q4_0` (~200 MB) on first run via
//! `CeraEngine::from_bundle_id`, caches it under
//! `target/tmp/cera-parity-cache/`, and asserts that running the same
//! prompt through `cera::CeraEngine` (the rust leg) and through
//! `cera_ffi::CeraEngine` (the ffi leg) yields byte-identical greedy
//! token streams.
//!
//! Gating mirrors `cera/tests/shift_real_model.rs`:
//!   - `#[ignore]` so default `cargo test` doesn't trigger a 200 MB
//!     download.
//!   - `CERA_PARITY_RUN=1` env var must be set; otherwise the test
//!     prints a skip line and returns. Belt-and-suspenders for CI
//!     where `--ignored` is passed but the gate isn't intentionally
//!     opted into.
//!
//! Manual invocation:
//!   `CERA_PARITY_RUN=1 cargo test -p cera-parity -- --ignored`
//!
//! CI invocation: see the `Parity Harness (gated)` job in
//! `.github/workflows/ci.yml`.

#[test]
#[ignore = "downloads ~200 MB; set CERA_PARITY_RUN=1 and pass --ignored"]
fn rust_and_ffi_produce_identical_tokens() {
    if std::env::var("CERA_PARITY_RUN").is_err() {
        eprintln!("skipping: CERA_PARITY_RUN not set");
        return;
    }

    let cache = cera_parity::default_cache_dir().expect("cache dir");
    let args = cera_parity::RunArgs {
        bundle: "LFM2-350M-Extract-GGUF",
        quant: "Q4_0",
        prompt: "The capital of France is",
        max_tokens: 16,
        seed: 0,
        cache_dir: &cache,
    };

    let (rust, rust_ms) = cera_parity::run_rust(&args).expect("rust path");
    let (ffi, ffi_ms) = cera_parity::run_ffi(&args).expect("ffi path");
    eprintln!("perf: rust={rust_ms:?} ffi={ffi_ms:?}");

    // Greedy decoding is deterministic across runs; the rust and ffi
    // legs share the same library underneath so they MUST agree
    // token-for-token. Divergence here means the FFI wrapper's
    // `RecordType -> cera::Type` adapter is dropping or reordering
    // something that influences sampling (seed, temperature, top_k).
    if let Some(idx) = cera_parity::first_divergence(&rust, &ffi) {
        // Clamp each window to its own slice's length so a tail
        // divergence (where one leg ran out first) still prints the
        // surrounding tokens instead of returning `None` from
        // `slice.get(range)`.
        let start = idx.saturating_sub(2);
        let end = idx.saturating_add(3);
        let rust_window = start..end.min(rust.len());
        let ffi_window = start..end.min(ffi.len());
        let rust_dump = format!("{:?}", &rust[rust_window.clone()]);
        let ffi_dump = format!("{:?}", &ffi[ffi_window.clone()]);
        panic!(
            "rust ↔ ffi diverged at index {idx}\n  rust[{rust_window:?}] = {rust_dump}\n  ffi [{ffi_window:?}] = {ffi_dump}\n  rust.len() = {}, ffi.len() = {}",
            rust.len(),
            ffi.len(),
        );
    }
    assert!(
        rust.len() <= 16,
        "honored max_tokens cap (got {} tokens)",
        rust.len()
    );
    assert!(
        !rust.is_empty(),
        "expected at least one decoded token from a non-empty prompt"
    );
}
