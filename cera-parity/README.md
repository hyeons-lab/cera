...
3. **Real model, real download.** Synthetic GGUFs would short-circuit
   ...
   `LiquidAI/LeapBundles` HF repo via [`CeraEngine::from_bundle_id`]
...
cargo run -p cera-parity -- dump --via rust \
...
cargo run -p cera-parity -- check \
    --bundle LFM2-350M-Extract-GGUF --quant Q4_0 \
    --prompt "The capital of France is" \
    --max-tokens 16 \
    --kotlin-runner $(pwd)/cera-parity/legs/kotlin/build/libs/cera-parity-kotlin-all.jar \
    --swift-runner  $(pwd)/cera-parity/legs/swift/.build/release/CeraParitySwift
#
...
The harness uses `target/tmp/cera-parity-cache/` under the workspace
root by default. Override with `--cache-dir <path>` (CLI) or
`$CERA_PARITY_CACHE_DIR` (env). Distinct from `cera-test-models/`
(used by `cera`'s `tests/common/download.rs`) so the two caches can
...
```bash
cd cera-parity/legs/kotlin && ./gradlew shadowJar && cd -
cargo build -p cera-ffi   # produces target/debug/libcera_ffi.{so,dylib}
CERA_PARITY_RUN=1 \
  CERA_PARITY_KOTLIN_RUNNER=$(pwd)/cera-parity/legs/kotlin/build/libs/cera-parity-kotlin-all.jar \
  cargo test -p cera-parity --test parity_kotlin -- --ignored
```
