...
//! (Rust direct, Rust through cera-ffi, Kotlin via JNA, Swift via
...
//!
//! Four legs ship today: `run_rust` (calls `cera::CeraEngine`
//! directly), `run_ffi` (calls `cera_ffi::CeraEngine` through its
//! Rust public surface), `run_kotlin_jna` (spawns a vendored Kotlin
//! runner that loads the generated `cera_ffi.kt` bindings and
//! `libcera_ffi.{so,dylib}` through JNA), and `run_swift_uniffi`
//! (spawns a vendored Swift runner built via SPM that loads the
//! generated `cera_ffi.swift` bindings and `libcera_ffi.dylib`
//! linked at build time). The first two paths produce identical
//! output by construction — the cera-ffi wrapper is a thin Rust
...
/// `$CERA_PARITY_CACHE_DIR` if set, else
/// `<workspace-root>/target/tmp/cera-parity-cache/`. Distinct from
/// `cera-test-models/` (used by `cera`'s integration tests) so
...
pub fn default_cache_dir() -> Result<PathBuf> {
    if let Ok(override_path) = std::env::var("CERA_PARITY_CACHE_DIR") {
        let p = PathBuf::from(override_path);
        std::fs::create_dir_all(&p)
            .with_context(|| format!("create cache dir at {}", p.display()))?;
        return Ok(p);
    }
    // CARGO_MANIFEST_DIR points at this crate (cera-parity); workspace
    // root is one level up. Same convention as cera's
...
/// `cera-parity/legs/kotlin/.../Main.kt` and
/// `cera-parity/legs/swift/Sources/CeraParitySwift/main.swift`. There
...
/// Greedy-decoding generation options against the cera-core API.
/// Constructed from the shared `settings` constants so the
/// cera-core ↔ cera-ffi pair can't drift on a knob change.
...
/// Same options, expressed against the cera-ffi record. Sister
...
fn engine_config_with_repo(cache_dir: &Path) -> cera::EngineConfig {
    cera::EngineConfig {
        // `as usize`: `CONTEXT_SIZE` is the FFI wire-width (`u64`); the
        // cera-core API stores it as `usize`. Lossless because the
...
/// Run the prompt through `cera::CeraEngine` directly. Reference leg —
...
pub fn run_rust(args: &RunArgs<'_>) -> Result<(Vec<u32>, Option<u64>)> {
    ...
    let engine = cera::CeraEngine::from_bundle_id(args.bundle, args.quant, cfg)
...
/// Run the prompt through `cera_ffi::CeraEngine`'s Rust public
/// surface. Same library underneath — this leg verifies the
/// `RecordType <-> cera::Type` adapters round-trip without dropping
...
pub fn run_ffi(args: &RunArgs<'_>) -> Result<(Vec<u32>, Option<u64>)> {
    ...
    let cache_dir_str = args.cache_dir.to_str().with_context(|| {
        format!(
            "cache_dir {} is not valid UTF-8 (cera-ffi requires String)",
            args.cache_dir.display()
        )
    })?;
    let started = Instant::now();
    let repo = cera_ffi::BundleRepo::new(cache_dir_str.to_owned());
    let cfg = cera_ffi::EngineConfig {
    ...
    let session_cfg = cera_ffi::SessionConfig {
        seed: Some(args.seed),
        ..Default::default()
    };
...
/// `cera-parity/legs/swift/`. Spawns the SPM-built executable
/// (`.build/release/CeraParitySwift`), feeds it a `RunArgsOwned`-shaped
...
/// directory containing `libcera_ffi.dylib`.
///
...
/// `libcera_ffi.dylib` for the linker), and dyld can resolve via
