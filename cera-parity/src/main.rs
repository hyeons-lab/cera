...
//!
//! ```text
//! cera-parity dump --via {rust|ffi|kotlin-jna|swift-uniffi} \
//!     --bundle <id> --quant <q> --prompt <text> \
//!     [--max-tokens 16] [--seed 0] \
//!     [--runner <jar-or-bin>] [--lib-dir <dir>]
//!
//! cera-parity check --bundle <id> --quant <q> --prompt <text> \
//!     [--max-tokens 16] [--seed 0] \
...
enum Cmd {
    ...
    Dump {
        ...
        /// (`cera-parity/legs/kotlin/build/libs/cera-parity-kotlin-all.jar`)
        /// or the Swift SPM binary
        /// (`cera-parity/legs/swift/.build/release/CeraParitySwift`).
        /// Required when `--via kotlin-jna` or `--via swift-uniffi`;
        /// passing this flag with any other `--via` value is rejected
        /// as a misuse.
        #[arg(long)]
        runner: Option<PathBuf>,
        /// Directory containing `libcera_ffi.{so,dylib}`. Defaults to
    ...
    Check {
        ...
        /// Directory containing `libcera_ffi.{so,dylib}`. Defaults
...
enum Leg {
    /// `cera::CeraEngine` directly.
    #[value(name = "rust")]
    Rust,
    /// `cera_ffi::CeraEngine` through its Rust public surface.
    #[value(name = "ffi")]
    Ffi,
    /// Kotlin runner under `cera-parity/legs/kotlin/`, loading the
    /// generated UniFFI bindings + `libcera_ffi.{so,dylib}` via JNA.
    #[value(name = "kotlin-jna")]
    KotlinJna,
    /// Swift runner under `cera-parity/legs/swift/`, built via SPM,
    /// loading the generated UniFFI Swift bindings + linked against
    /// the cera-ffi cdylib at build time.
...
struct CommonArgs {
    ...
    /// `$CERA_PARITY_CACHE_DIR` and then to
    /// `<workspace>/target/tmp/cera-parity-cache/`.
    #[arg(long)]
