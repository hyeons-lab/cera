...
// generated UniFFI Kotlin bindings end-to-end (via JNA → libcera_ffi),
// emits a `RunOutput` JSON document on stdout, and exits 0 on success
// or 1 on any error (with a one-line `error: <msg>` to stderr).
//
// Constants here MUST mirror `cera_parity::settings` in
// `cera-parity/src/lib.rs` — drift between the two would surface as
// false parity failures that aren't actually FFI bugs. Keep them in
// sync byte-for-byte.

package com.hyeonslab.ceraparity
...
// Mirror of `cera_parity::settings`. Manual copy (no shared schema)
...
private fun runOnce(args: RunArgsOwned): List<UInt> {
    ...
    val engine = CeraEngine.fromBundleId(args.bundle, args.quant, cfg)

    // `ubatchSize = 512u` mirrors `cera::SessionConfig::default()` —
    // the Rust legs in `cera-parity/src/lib.rs` build their session
