"""
Export trained DSpark PyTorch checkpoint to GGUF format compatible with Cera and llama.cpp.
...
def export_to_gguf(
    ...
    if is_markov_model:
        ...
        # Write linear feature projection fc.weight
        if "fc.weight" in state_dict:
            fc_w = state_dict["fc.weight"].float().numpy()
            gguf_writer.add_tensor("fc.weight", fc_w)
    ...
    for l in range(num_layers):
        ...
        if f"{pfx}.attn_norm.weight" in state_dict:
            attn_norm = state_dict[f"{pfx}.attn_norm.weight"].float().numpy()
            attn_q = state_dict[f"{pfx}.attn_q.weight"].float().numpy()
            attn_k = state_dict[f"{pfx}.attn_k.weight"].float().numpy()
            attn_v = state_dict[f"{pfx}.attn_v.weight"].float().numpy()
            attn_output = state_dict[f"{pfx}.attn_output.weight"].float().numpy()

            ffn_norm = state_dict[f"{pfx}.ffn_norm.weight"].float().numpy()
            ffn_gate = state_dict[f"{pfx}.ffn_gate.weight"].float().numpy()
            ffn_up = state_dict[f"{pfx}.ffn_up.weight"].float().numpy()
            ffn_down = state_dict[f"{pfx}.ffn_down.weight"].float().numpy()

            gguf_writer.add_tensor(f"blk.{l}.attn_norm.weight", attn_norm)
            gguf_writer.add_tensor(f"blk.{l}.attn_q.weight", attn_q)
            gguf_writer.add_tensor(f"blk.{l}.attn_k.weight", attn_k)
            gguf_writer.add_tensor(f"blk.{l}.attn_v.weight", attn_v)
            gguf_writer.add_tensor(f"blk.{l}.attn_output.weight", attn_output)
            ...
            gguf_writer.add_tensor(f"blk.{l}.ffn_norm.weight", ffn_norm)
            gguf_writer.add_tensor(f"blk.{l}.ffn_gate.weight", ffn_gate)
            gguf_writer.add_tensor(f"blk.{l}.ffn_up.weight", ffn_up)
            gguf_writer.add_tensor(f"blk.{l}.ffn_down.weight", ffn_down)

    # Output norm
    if "output_norm.weight" in state_dict:
        norm_w = state_dict["output_norm.weight"].float().numpy()
        gguf_writer.add_tensor("output_norm.weight", norm_w)
    elif "norm.weight" in state_dict:
        norm_w = state_dict["norm.weight"].float().numpy()
        gguf_writer.add_tensor("output_norm.weight", norm_w)

    print(f"Writing GGUF sidecar ({arch_name}) to {output_gguf_path}...")
    gguf_writer.write_header_to_file()
    gguf_writer.write_kv_data_to_file()
    gguf_writer.write_tensors_to_file()
    gguf_writer.close()
    print(f"GGUF sidecar export complete: {output_gguf_path}")

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python convert_gguf.py <input_ckpt.pt> <output.gguf>")
    else:
        export_to_gguf(sys.argv[1], sys.argv[2])
