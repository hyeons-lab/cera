...
void main() {
  ...
  group('ChatController transactional chat coordinator', () {
    ...
    test('coordinates Turn 1 initial, Turn 2 continuation, interruption and reset', () async {
      ...
      expect(turn1Messages.any((m) => m.role == 'user' && m.content == 'Hello'), isTrue);
      expect(fakeCera.generatedPrompts.length, 1);
      expect(fakeCera.generatedPrompts[0], contains('<|im_start|>system'));
      expect(fakeCera.generatedPrompts[0], contains('<|im_start|>user\nHello<|im_end|>'));
      ...
      expect(
        fakeCera.generatedPrompts[1].startsWith('<|im_end|>\n<|im_start|>user\nHow are you?'),
        isTrue,
      );

      // Turn 3: Interrupted turn
      fakeCera.pauseGeneration = true;
      final sendFuture = controller.dispatch(const SendMessageIntent('Tell me a story'));
      ...
      expect(
        fakeCera.generatedPrompts[3].startsWith('<|im_end|>\n<|im_start|>user\nContinue'),
        isTrue,
      );
      ...
      expect(turn5Messages.any((m) => m.role == 'user' && m.content == 'Fresh start'), isTrue);

      controller.dispose();
    });

    test('marks phase unusable on reset failure, preserves turns, and refuses further dispatches', () async {
    ...
    test('trims leading newline after stripping BOS marker in continuation turns', () async {
      ...
      expect(fakeCera.generatedPrompts[1].startsWith('<|im_end|>\n<|im_start|>user'), isTrue);
      expect(fakeCera.generatedPrompts[1].startsWith('<|im_end|>\n\n<|im_start|>user'), isFalse);
...
class TestModelSource extends LoadedModel {
  ...
  @override
  Future<Cera> open({CeraOptions options = const CeraOptions()}) async => engine;
}
...
class FakeCera implements Cera {
  ...
  Future<String> applyChatTemplate(
    ...
    final buf = StringBuffer(prefixWithNewline ? '<|startoftext|>\n' : '<|startoftext|>');
